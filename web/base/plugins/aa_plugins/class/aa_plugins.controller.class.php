...
var $lang = array(
	...
	'enable' => array (
		...
		'error_enabled' => 'Plugin %s allready installed',
	...
	'disable' => array (
		...
		'error_in_use' => 'Plugin %s is still in use by htvcenter',
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new aa_plugins_select($this->htvcenter, $this->response);
	...
	function configure( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new aa_plugins_configure($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
