...
var $lang = array();
	...
	function __construct($htvcenter, $response) {
		...
		require_once($this->htvcenter->get('classdir').'/htvcenter_server.class.php');
	}
	...
	function configure() {
		...
		$tmp  = $this->htvcenter->get('basedir').'/var/lock/plugin_manager.'.$this->plugin.'.conf.tmp';
		$file = $this->htvcenter->get('basedir').'/var/lock/plugin_manager.'.$this->plugin.'.conf';

		$command  = $this->htvcenter->get('basedir').'/bin/htvcenter boot-service view';
		$command .= ' --htvcenter-ui-user '.$this->user->name;
		$command .= ' --htvcenter-cmd-mode raw';
		...
		if(!$form->get_errors() && $response->submit()) {
			...
			foreach($request as $k => $v) {
				if($v === '') { $v = 'NoValue'; }
				$command  = $this->htvcenter->get('basedir').'/bin/htvcenter boot-service configure';
				$command .= ' --htvcenter-ui-user '.$this->user->name;
	...
	function __parse_conf ( $path, $plugin = null ) {
		if(file_exists($path)) {
			...
			foreach( $ini as $line ){
				$line = trim( $line );
				// only get lines starting with htvcenter_ or $plugin_
				if(strpos($line, 'htvcenter_') === 0 || (isset($plugin) && stripos($line, str_replace('-', '_', $plugin)) === 0)) { 
					// Key-value pair
