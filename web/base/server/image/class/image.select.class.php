	...
	function select() {
		...
		foreach ($image_arr as $index => $image_db) {
			...
			//	$a->handler = 'onclick="wait();"';
				...
				// set the active icon
				$isactive_icon = '<div class="resource_state"><span class="pill inactive">'.$this->lang['lang_inactive'].'</span></div>';
				if ($image_db["image_isactive"] == 1) {
					$isactive_icon = '<div class="resource_state"><span class="pill active">'.$this->lang['lang_active'].'</span></div>';
				} else {
				...
				$ln = strlen($image_db["image_name"]);

				if ($ln > 32) {
					$imgname = substr($image_db["image_name"],0,32);
					$imgname .= '...';
				} else {
					$imgname = $image_db["image_name"];
				}
				$data  = '<div class="data">';
				$data .= '<b>'.$this->lang['table_id'].'</b>: <span class="imageid">'.$image_db["image_id"].'</span><br>';
				$data .= '<b>'.$this->lang['table_name'].'</b>: '.$imgname.'<br>';
				...
				$b[] = array(
					'image_isactive' => '	<div class="panel-heading appnamer">
									<h3 class="panel-title">'.$imgname.'</h3></div>'.$isactive_icon,
		...
		// Filter
		$types = $this->htvcenter->deployment();
		$list = $types->get_list();
		$filter = array();
		foreach( $list as $l) {
