...
var $lang = array();
		...
		if($id !== '') {
			...
			if(!$form->get_errors() && $this->response->submit()) {
				...
				if(strlen($image_password)) {
					...
					$CMD="rm -f ".$this->htvcenter->get('basedir')."/web/action/image-auth/iauth.".$id.".php";
					exec($CMD);
				}
				// install-from-nfs
	...
	function get_response() {
		...
		$storage_resource = $this->htvcenter->resource();
		...
		if ($rootfs_local_deployment_enabled) {
			...
			foreach($deployment_id_arr as $deployment_id) {
				...
				$local_deployment_templates_identifier_hook = $this->htvcenter->get('basedir')."/web/boot-service/template.".$local_deployment->type.".php";
