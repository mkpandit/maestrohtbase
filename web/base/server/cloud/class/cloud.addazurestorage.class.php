...
class addazurestorage {
/**
* name of action buttons
* @access public
* @var string
*/
var $actions_name = 'addazurestorage';
/**
* message param
* @access public
* @var string
*/
var $message_param = "addazurestorage";
/**
* id for tabs
* @access public
* @var string
*/
var $prefix_tab = 'addazurestorage';
/**
* identifier name
* @access public
* @var string
*/
var $identifier_name = 'addazurestorage';
/**
* path to templates
* @access public
* @var string
...
var $lang = array();
	...
	function action() {
		...
		if(isset($response->msg)) {
			$this->response->redirect(
				$this->response->get_url($this->actions_name, 'azurestorage', $this->message_param, $response->msg)
			);
		}
		if(isset($response->error)) {
			$_REQUEST[$this->message_param] = $response->error;
		}
		$t = $this->response->html->template($this->tpldir.'/cloud-add-azure-storage.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command = shell_exec('python '.$this->rootdir.'/server/cloud/script/createazurestorage.py '.$azure_storage_name. ' '.$azure_resource_group);
			...
			if(empty($data)) {
				$response->msg = sprintf("Azure Storage not created");
			} else {
	...
	function get_response() {
		...
		$form = $response->get_form($this->actions_name, 'addazurestorage');
		$submit = $form->get_elements('submit');
		$submit->handler = 'onclick="wait();"';
		$submit->value = 'Add Azure Storage Account';
		...
		$d['azure_storage_name']['object']['type']                    	= 'htmlobject_input';
		...
		$d['azure_storage_name']['object']['attrib']['value']         	= $azure_vm_name;
		$d['azure_storage_name']['object']['attrib']['maxlength']     	= 50;
