...
var $lang = array();
	...
	function action() {
		$data = $this->disks();
		$t = $this->response->html->template($this->tpldir.'/cloud-aws-volumes.tpl.php');
	...
	function disks() {
		...
		if(isset($_GET) && !empty($_GET['ec2_id'])){
			...
			$html_information = "";
			$volume_info_dump = shell_exec('sudo python '.$this->rootdir.'/server/cloud/script/awsvolumes.py');
			$volume_info = json_decode($volume_info_dump, true);
			if(empty($volume_info)){
				$html_information .= "<div class='nothing-found'>Currenly no vloume available on AWS</div>";
			} else {
				$count = 1;
				foreach($volume_info as $k => $v){
					$temp = explode("_*_", $v);
					$html_information .= "<div class='cloud-item item-".$count."'><h4>".$temp[0]."</h4><i class='fa fa-database fa-3'></i>";
					$html_information .= "<div class='cloud-properties'><br />Size: ".$temp[1]." GB, State: ".$temp[2]."<br />Type: ".$temp[3]."</div>";
					$html_information .= "</div>";
					$count++;
				}
			}
