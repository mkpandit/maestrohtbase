...
var $lang = array();
	...
	function select() {
		...
		foreach ($storages as $key => $value) {
			...
			#$url  = $this->htvcenter->get('baseurl').'/index.php?plugin='.$deployment->storagetype.'&'.str_replace('-', '_',$deployment->storagetype).'_action=edit&storage_id='.$value["storage_id"];
			...
			if( $deployment->storagetype === 'equallogic-storage' ||
				...
				$url = $this->htvcenter->get('baseurl').'/index.php?plugin='.$deployment->storagetype.'&currenttab=tab0&action=select&identifier[]='.$storage->id;
			}
			$a = $this->response->html->a();
			...
			if (
				...
				$b[] = array(
					'storage_state' => '<div class="panel-heading appnamer">
									<h3 class="panel-title">'.$value["storage_name"].'</h3>
								</div>'.$state_icon,
