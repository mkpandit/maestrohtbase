...
var $lang = array(
	...
	'new' => array (
		'label' => 'New resource',
		'form_docu' => 'This form is to manually create a new resource which htvcenter cannot directly monitor<br>
			via the htvcenter-client e.g. NetApp Filers or EqualLogig Storages.<br>
			The resource then can still be used as Storage Server managed by htvcenter.',
		'form_auto_resource' => 'Resources (physical systems and virtual machines) intended for rapid deployment<br>
			 are automatically added to htvcenter by setting their bios to PXE/Net-Boot.<br>
...
//		$response->html->debug();
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new resource_select($this->htvcenter, $this->response);
	...
	function resource_new( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new resource_new($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new resource_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
