...
var $lang = array();
	...
	function action() {
		...
		$i = 0;
		foreach ($plugins->__elements as $plugin) {
			switch($i) {
				case 0:
					$colorpan = 'panel-success';
				break;
			...
			$myplugins .= '<a href="'.$plugin->href.'" id="'.$idcss.'"><div class="panel '.$colorpan.' panel-colorful text-center">
										<div class="panel-body">
												<i class="fa '.$faico.' fa-2x"></i>
										</div>
										<div class="bg-light pad-ver">
											<h4 class="mar-no text-thin">'.$plugin->label.'</h4>
										</div>
									</div></a>';
		...
		$myplugins .='<span id="appidval">'.$this->response->params['appliance_id'].'</span>';
...
*/
	...
	function get_response() {
		...
		if(isset($hide_virtualization)) {
			...
			$div->name = '';
			$virrt = $virtualization->name;
			$virrt = str_replace('KVM', 'OCH', $virrt);
			$virrt = str_replace('(localboot)', '', $virrt);
 			$div->add($virrt);
		...
		$d['comment']['validate']['regex']             = $this->htvcenter->regex['comment'];
		$d['comment']['validate']['errormsg']          = sprintf($this->lang['error_comment'], $this->htvcenter->regex['comment']);
...
*/
		...
		foreach ($plugins as $k => $v) {
			...
			$link = '';
			$hook = $this->htvcenter->get('webdir')."/plugins/".$v."/htvcenter-".$v."-appliance-edit-hook.php";
			if ($this->file->exists($hook)) {
				require_once($hook);
				$function = str_replace("-", "_", 'get_'.$v.'_appliance_edit');
				if(function_exists($function)) {
					$id = $this->apliance_id;
					$link = $function($id, $this->htvcenter, $this->response);
					if(is_object($link)) {
