...
var $lang = array();
	...
	function select() {
		...
		$table->identifier_disabled = array( 'htvcenter' );
	...
	function get_response($mode) {
		...
		if( $mode !== 'select' && $mode !== 'delete') {	
			...
			$lang = array();
			$files = $this->htvcenter->file()->get_files($this->htvcenter->get('basedir').'/web/base/lang/', '', '*.htmlobjects.ini');
			foreach($files as $v) {
				$tmp = explode('.', $v['name']);
				$lang[] = array($tmp[0]);
			}
			...
			if(
				...
				if(is_array($roles)) {
					...
					if($mode === 'edit') {
                        ...
                        // if ldap is enabled do not allow access the the htvcenter user administration
			if (file_exists($this->htvcenter->get('basedir').'/plugins/ldap/web/.running')) {
				$d['pass1'] = '';
				$d['pass2'] = '';
			} else {
