	...
	function select() {
		...
		switch ($this->response->html->request()->get('event_filter')) {
			...
			case 'warning':
				$events = $event->display_overview($table->offset, $table->limit, $table->sort, $table->order);
				$filtevent = array();
				foreach ($events as $filt) {
					if ($filt['event_priority'] == 5) {
						$filtevent[] = $filt;
					}
				}
				$events = $filtevent;
		...
		foreach ($events as $key => $value) {
			...
			switch ($value['event_priority']) {
					case 0:
						$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-danger"><i class="fa fa-power-off fa-lg"></i></span>'; break;
					case 1:
					case 2:
					case 3:
						$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-danger"><i class="fa fa-exclamation-triangle fa-lg"></i></span>'; break; // error event
					case 4:
					case 5:
					case 6:
					case 7:
					case 8:
						$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-warning"><i class="fa fa-bell fa-lg"></i></span>'; break; // undefined event
					case 9:	
						$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-success"><i class="fa fa-play fa-lg"></i></span>'; break; // active event
					case 10:
						$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-success"><i class="fa fa-smile-o fa-lg"></i></span>'; break; // notice event
			}
			...
			if (strstr($value['event_description'], "ERROR running token")) {
				...
				if (($this->file->exists($cmd_file)) && ($this->file->exists($error_file))) {
					...
					$icon = '<span class="icon-wrap icon-wrap-sm icon-circle bg-danger"><i class="fa fa-exclamation-triangle fa-lg"></i></span>';
					// set the description
			...
			$b[$i]['event_time'] = '<span class="text-semibold">'.$value['event_source'].'</span>
													<br>
													<small class="text-muted">'.date('Y/m/d H:i:s', $value['event_time']).'</small>'; 
			//$b[$i]['event_source'] ='';
		...
		$select->handler = 'onchange="wait();filterevents();return false;"';
		...
		$table->id = 'Tabelle';
		$table->css = 'table table-hover table-vcenter';
		$table->border = 1;
		$table->cellspacing = 0;
