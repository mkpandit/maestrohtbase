...
// - ThemeOn.net -
 ...
 $(document).ready(function() {
	...
	var storagearray = []
	$('.gaugestorage').each(function(i) {
			
		 	var target = document.getElementById('gauge-'+i); // your canvas element
		 	var maxval = $(this).find('.sizestor').text();
		 	var setval = $(this).find('.sizeused').text();
		 	maxval = parseInt(maxval);
		 	setval = parseInt(setval);
		 	storagearray[i] = setval;
			var gauge = new Gauge(target).setOptions(opts); // create sexy gauge!
			gauge.maxValue = maxval; // set max gauge value
			gauge.animationSpeed = 32; // set animation speed (32 is default value)
			gauge.set(setval); // set actual value
			gauge.setTextField(document.getElementById("gauge-txt-"+i));
	...
	function gaugeupdate() {
			...
			$('.gaugestorage').each(function(i){
				var target = document.getElementById('gauge-'+i); // your canvas element
			 	var maxval = $(this).find('.sizestor').text();
			 	maxval = parseInt(maxval);
			 	setval = storagearray[i];
			 	var urlg = window.location.href;
			 	var id = $(this).attr('idstor');
			 	console.log(urlg);
			 	$.ajax({
				  type: "POST",
				  url: urlg,
				  ...
				  success: function(data){
				  			...
				  			var newset = parseInt(data);
				  		 	

				  		console.log(setval);
				  		console.log(newset);
				  		var step = newset - setval;
				  		storagearray[i] = newset;
