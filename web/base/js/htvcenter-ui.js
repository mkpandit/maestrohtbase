/**
 * This file contains misc htvcenter UI javascript methods
 *
...
$(document).ready(function(){
	...
	$('#menu1_1').prepend( $('<span>').addClass('fa fa-server sidericon').prepend($('<i>')) );
	$('#menu2_1').prepend( $('<span>').addClass('fa fa-puzzle-piece sidericon').prepend($('<i>'))	);
	...
	// add htvcenter.wait event to listed elements
	// list of interface elements that should trigger the htvcenter.wait() function
	var waitElements = [
		'input.submit',
		'a.edit',
		'a.manage',
	...
	$.each(waitElements, function(k, v) {
		...
		$(v).click(function() { htvcenter.wait(v ,'') } );
	});
	...
	$(namegen).click(function() {
		htvcenter.generateUniqueName($('input.namegen').attr('data-prefix'), $('input.namegen'), $('input.namegen').attr('data-length'));
		return false;
	});
...
/* global htvcenter interface methods */
htvcenter = {
	...
	wait : function(element, msg) {
		...
		$('body').prepend(
			$('<div>').attr('class', 'modal-box lead').append(
					$('<p>').html(msg)
				)
				/*
				.append(
					$('<img>').attr({'src':'/htvcenter/base/img/ajax-loader.gif','class':'ajax-loader'})
				)
				*/
		);
	...
	*/
	...
	/* Format date helper.
		Usage:	htvcenter.formatDate([timestamp], '%Y-%M-%d')
		date:	unix timestamp
		fmt:	format string
	*/
