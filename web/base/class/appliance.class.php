...
// This class represents an applicance managed by htvcenter
...
function add($appliance_fields) {
	...
	if (! $result) {
		...
		foreach ($enabled_plugins as $index => $plugin_name) {
			$plugin_start_appliance_hook = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-appliance-hook.php";
...
// for the wizard we have to provide an add method without running the hook in step 1
...
function update($appliance_id, $appliance_fields) {
	...
	if (! $result) {
		...
		$enabled_plugins = $plugin->enabled();
		foreach ($enabled_plugins as $index => $plugin_name) {
			$plugin_update_appliance_hook = $RootDir."/plugins/".$plugin_name."/htvcenter-".$plugin_name."-appliance-hook.php";
...
function stop() {
	...
	$enabled_plugins = $plugin->enabled();
	foreach ($enabled_plugins as $index => $plugin_name) {
		$plugin_stop_appliance_hook = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-appliance-hook.php";
...
function get_list() {
	...
	$appliance_name_array = htvcenter_db_get_result_double ($query);
