...
var $lang = array(
	'step1' => array (
		...
		'howto' => 'The selected network card will be used to setup HyperTask Server and create
					the HyperTask Management Network. All available and configured network interfaces
	...
	'step2' => array (
		...
		'howto' => 'Select the database type to use for storing the HyperTask data.',
		'error_empty' => 'Database type may not be empty',
		'saved' => 'Saved database type %s',
	),
	'step3' => array (
		'tab' => 'Step 3',
		'label' => 'Configure the database connection and initialize HyperTask',
		'form_server' => 'Server',
		'form_db' => 'Database',
		'form_user' => 'User',
		...
		'saved' => 'Successfully initialized HyperTask',
		'failed' => 'Failed to initialyze HyperTask! Please check syslog!',
	),
);
	...
	function __construct($response, $file) {
		...
		if (($this->file->exists("/etc/init.d/htvcenter")) && (is_link("/etc/init.d/htvcenter"))) {
		...
		if($visible === true) {
			...
			if(!$form->get_errors() && $this->response->submit()) {
				...
				if($nic === '') {
					...
					$command = $this->basedir."/sbin/htvcenter-exec -i 127.0.0.1 -l true -t $token -c \"sed -i -e 's/^htvcenter_SERVER_INTERFACE=.*/htvcenter_SERVER_INTERFACE=\"$nic\"/g' ".$this->basedir."/etc/htvcenter-server.conf\"";
					shell_exec($command);
	...
	function step3( $visible = false ) {
		...
		if($visible === true) {
			if($dbtype === '') {
				...
				$d['param_db_2']['object']['attrib']['value']  = 'htvcenter';
				...
				if(!$form->get_errors() && $this->response->submit()) {
					...
					$token = md5(uniqid(rand(), true));
					$command  = $this->basedir.'/sbin/htvcenter-exec -i 127.0.0.1 -l true -t '.$token.' -c';
					$command .= ' "sed -i -e \'s/^htvcenter_DATABASE_TYPE=.*/htvcenter_DATABASE_TYPE="'.$type.'"/g\'';
					$command .= ' -e \'s/^htvcenter_DATABASE_SERVER=.*/htvcenter_DATABASE_SERVER="'.$server.'"/g\'';
					$command .= ' -e \'s/^htvcenter_DATABASE_NAME=.*/htvcenter_DATABASE_NAME="'.$db.'"/g\'';
					$command .= ' -e \'s/^htvcenter_DATABASE_USER=.*/htvcenter_DATABASE_USER="'.$user.'"/g\'';
					$command .= ' -e \'s/^htvcenter_DATABASE_PASSWORD=.*/htvcenter_DATABASE_PASSWORD="'.$pass.'"/g\'';
					if (!strcmp($type, "oracle")) { 
						$command .= ' -e \'s/#htvcenter_LD_LIBRARY_PATH=.*/htvcenter_LD_LIBRARY_PATH="'.$oqc_db_ld_path.'"/g\'';
						$command .= ' -e \'s/#htvcenter_ORACLE_HOME=.*/htvcenter_ORACLE_HOME="'.$oqc_db_home.'"/g\'';
						$command .= ' -e \'s/#htvcenter_TNS_ADMIN=.*/htvcenter_TNS_ADMIN="'.$oqc_db_tns.'"/g\'';
						$command .= ' -e \'s/htvcenter_LD_LIBRARY_PATH=.*/htvcenter_LD_LIBRARY_PATH="'.$oqc_db_ld_path.'"/g\'';
						$command .= ' -e \'s/htvcenter_ORACLE_HOME=.*/htvcenter_ORACLE_HOME="'.$oqc_db_home.'"/g\'';
						$command .= ' -e \'s/htvcenter_TNS_ADMIN=.*/htvcenter_TNS_ADMIN="'.$oqc_db_tns.'"/g\'';
					}
					$command .= ' '.$this->basedir.'/etc/htvcenter-server.conf"';
					shell_exec($command);
					// send to sleep to wait for background command
					sleep(4);
					...
					// restore last backup ?
					$command = $this->basedir."/sbin/htvcenter-exec -i 127.0.0.1 -l true -t ".$token." -c \"".$this->basedir."/bin/htvcenter init_config";
					if ($restore !== '') {
						$command .= " restore\"";
					} else {
						$command .= "\"";
