...
// This class represents a resource in htvcenter (physical hardware or virtual machine)
/*
...
function get_instance($id, $mac, $ip) {
	...
	if ("$id" != "") {
		...
		//echo "select * from $RESOURCE_INFO_TABLE where resource_mac='$mac_lowercase' or resource_mac='$mac_uppercase'";
	...
	if(isset($resource_array)) {
		...
		foreach ($resource_array as $index => $resource) {
			...
			$this->htvcenterserver = $resource["resource_htvcenterserver"];
...
function add($resource_fields) {
	...
	global $RESOURCE_INFO_TABLE;
	global $htvcenter_RESOURCE_BASE_DIR;
	global $event;
	global $RootDir;
	...
	# set defaults
	$resource_fields["resource_basedir"]=$htvcenter_RESOURCE_BASE_DIR;
	$resource_fields["resource_htvcenterserver"]=$htvcenter_SERVER_IP_ADDRESS;
	$resource_fields["resource_execdport"]=$htvcenter_EXEC_PORT;
	...
	foreach ($enabled_plugins as $index => $plugin_name) {
		$plugin_new_resource_hook = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-resource-hook.php";
		if (file_exists($plugin_new_resource_hook)) {
			...
			$resource_function="htvcenter_"."$plugin_name"."_resource";
...
function get_parameter($resource_id) {
	...
	if (!$recordSet)
		...
		if (isset($_GET['vip']) && !empty($_GET['vip'])) {
			if (preg_match('@\"@', $_GET['vip']) ){
				$recordSet->fields["resource_htvcenterserver"] = '"'.$_GET['vip'].'"';
			} else {
				$recordSet->fields["resource_htvcenterserver"] = $_GET['vip'];
			}
		}
	...
	echo "htvcenter_execution_layer=\"$htvcenter_EXECUTION_LAYER\"\n";
	// htvcenter server web protocol
	echo "htvcenter_web_protocol=\"$htvcenter_WEB_PROTOCOL\"\n";
	...
	echo "htvcenter_plugins=\"$plugin_list\"\n";
	echo "htvcenter_boot_services=\"$boot_service_list\"\n";
...
function get_fields($which) {
	...
	$resource_fields["resource_htvcenterserver"] = $resource->htvcenterserver;
...
function send_command($resource_ip, $resource_command, $command_timeout = NULL) {
	...
	switch($resource_command) {
		case 'reboot':
			...
			$enabled_plugins = $plugin->enabled();
			foreach ($enabled_plugins as $index => $plugin_name) {
				$plugin_start_resource_hook = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-resource-hook.php";
			...
			$plugin_resource_virtual_command_hook_vm_type = "$RootDir/plugins/$virtualization_plugin_name/htvcenter-$virtualization_plugin_name-resource-virtual-command-hook.php";
			...
			$plugin_resource_virtual_command_hook_image_type = "$RootDir/plugins/$deployment_plugin_name/htvcenter-$deployment_plugin_name-resource-virtual-command-hook.php";
	//            $plugin_resource_virtual_command_hook_image_type = "$RootDir/plugins/sanboot-storage/htvcenter-sanboot-storage-resource-virtual-command-hook.php";
			...
			if (strlen($plugin_resource_virtual_command_hook)) {
				...
				$resource_function="htvcenter_"."$virtualization_plugin_name"."_resource_virtual_command";
				$resource_function=str_replace("-", "_", $resource_function);
	...
	switch($htvcenter_EXECUTION_LAYER) {
		case 'dropbear':
			...
			$final_resource_command = "$htvcenter_SERVER_BASE_DIR/htvcenter/sbin/htvcenter-exec -i $resource_ip -t $cmd_token -c \"$resource_command\"";
		...
		case 'htvcenter-execd':
			$fp = fsockopen($resource_ip, $htvcenter_EXEC_PORT, $errno, $errstr, 30);
			if(!$fp) {
	...
	while (!$rs->EOF) {
		...
		if (("$resource_state" != "off") && ("$resource_lastgood" != "-1")) {
			if (($check_time - $resource_lastgood) > $RESOURCE_TIME_OUT) {
				...
				$enabled_plugins = $plugin->enabled();
				foreach ($enabled_plugins as $index => $plugin_name) {
					$plugin_ha_hook = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-ha-hook.php";
					if (file_exists($plugin_ha_hook)) {
						...
						$ha_function="htvcenter_"."$plugin_name"."_ha_hook";
