...
var $_event;
	...
	*/
	...
	function get_instance($id, $name, $type) {
		...
		if ("$id" != "") {
			...
			$this->_event->log("get_instance", $_SERVER['REQUEST_TIME'], 2, "deployment.class.php", "Could not create instance of deployment without data ".$error, "", "", 0, 0, 0);
			return;
		}
	...
	*/
	...
	function get_list() {
		...
		$deployment_name_array = htvcenter_db_get_result_double ($query);
	...
	*/
	...
	function get_storagetype_list() {
		...
		$ar_Return = array();
		$ar_tmp = array();
		$ar_result = htvcenter_db_get_result_double ($query);
	...
	*/
	...
	function get_id_by_storagetype($type) {
		...
		$ar_Return = array();
		$ar_Return = htvcenter_db_get_result_double($query);
		return $ar_Return;
	}
