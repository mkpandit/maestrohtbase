...
// based on PHP Layers Menu 3.2.0-rc (C) 2001-2004 Marco Pratesi - http://www.marcopratesi.it/
	...
	)
{
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {
		...
		$addclass = '';
				...
				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=vmware-esx&controller=vmware-esx-about&vmware_esx_about_action=usage') {
						$skip = 1;
				}


				if ( $this->tree[$cnt]['text'] == 'Collectd') {
						$skip = 1;
				}
				...
				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=network-manager&controller=network-manager-about') {
						$skip = 1;
				}

				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=device-manager&controller=device-manager-about') {
						$skip = 1;
				}
				...
				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=highavailability&controller=highavailability-about') {
						$skip = 1;
				}

				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=collectd&controller=collectd-about') {
						$skip = 1;
				}

				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=ip-mgmt&controller=ip-mgmt-about') {
					$skip = 1;
				}

				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=novnc&controller=novnc-about') {
					$skip = 1;
				}

				if ( $this->tree[$cnt]['parsed_href'] == 'index.php?plugin=local-server&controller=local-server-about') {
					$skip = 1;
				}
		...
		if (isset($this->tree[$cnt]['selected']) && $this->tree[$cnt]['selected']) {
			...
			if ($level > $lastlevel && $lastlevel != 0) {
				$this->_treeMenu[$menu_name] .= '<ul class="collapse">';
			} else {
				if ($level < $lastlevel) {
					$counul = $lastlevel - $level;
					for($i=0; $i<$counul; $i++) {
						$this->_treeMenu[$menu_name] .= '</ul>';
					}
				} 
		...
		if ($this->tree[$cnt]['text'] == 'Datacenter' || $this->tree[$cnt]['text'] == 'Plugins' ) {
			$this->_treeMenu[$menu_name] .='<li class="list-divider"></li>';
			$this->_treeMenu[$menu_name] .='<li class="list-header">'.$this->tree[$cnt]['text'].'</li>';
		...
		if ($skip !=1) {
			if ($addclass != '') {
				$this->_treeMenu[$menu_name] .= '
							<li id="'.$menu_name . $cnt . '" class="'.$linkstyle.' '.$addclass.'" >' . "\n";
			} else {
				$this->_treeMenu[$menu_name] .= '
							<li id="'.$menu_name . $cnt . '" class="'.$linkstyle.'" >' . "\n";
			}
		...
		/*
		Condition zum Ausschalten der leeren Menüeinträge (vorallem im Bereich "Plugins")
		Bedingungen:
		
			* Menüeinträge:
				- Aktueller Eintrag hat keinen Link: 'href' und/oder 'parsed_href' sind leer
				- Der aktuelle tree-level ist gleich dem des folgenden Elements
				- $cnt (Index-ID des aktuellen Eintrags) ist nicht '$this->_firstItem[$menu_name]'
			* Letzter Menüeintrag:
				- $cnt ist gleich dem letzten Menüeintrag
				- Der letzte Menüeintrag hat keinen Link
		if(!((empty($this->tree[$cnt]['href']) && $this->tree[$cnt]['level'] == $this->tree[$cnt+1]['level'] && $cnt != $this->_firstItem[$menu_name]) || ($cnt == $this->_lastItem[$menu_name] && empty($this->tree[$cnt]['href'])))
		)		
		*/	
		if(!((empty($this->tree[$cnt]['href']) && $this->tree[$cnt]['level'] == $this->tree[$cnt+1]['level'] && $cnt != $this->_firstItem[$menu_name]))) {
...
//$this->_treeMenu[$menu_name] .= '<img class="imgs" src="' . $img . '" alt="' . $alt . '">';
			...
			// corner at end of subtree or t-split
				if ($not_a_leaf) {
					...
					//$this->_treeMenu[$menu_name] .= '<span onmousedown="' . $toggle_function_name . "('".$menu_name . $cnt . "')" . '"><img class="imgs" id="'.$menu_name . $cnt . 'node" src="' . $img . '" alt="' . $alt . '" /></span>';
				} else {
				...
				if ($not_a_leaf) {
					...
					//$this->_treeMenu[$menu_name] .= '<img class="imgs" id="'.$menu_name . $cnt . 'node" src="' . $img . '" alt="' . $alt . '">';
				}
				...
				switch($this->tree[$cnt]['text']) {
					...
					case 'Plugin manager':
						...
						$label = '';
						$this->tree[$cnt]['text'] = 'Features control';
					break;
				...
				if ($this->tree[$cnt+1]['level'] > $level) {
				...
				if ( $this->tree[$cnt]['parsed_href'] == '/cloud-fortis') {
						$target_blank = 'target="_blank"';
				} 

				if ( $this->tree[$cnt]['parsed_href'] == 'plugins/cloud/soap/cloudadmin-soap-example-client.php') {
						$target_blank = 'target="_blank"';
				} 

				if ( $this->tree[$cnt]['parsed_href'] == '/cloud-fortis/user/soap/clouduser-soap-example-client.php') {
						$target_blank = 'target="_blank"';
				} 

				if ( $this->tree[$cnt]['parsed_href'] == '/cloud-nephos') {
						$target_blank = 'target="_blank"';
				} 
				...
				if ($skip != 1) {
					$a_href_open = '<a href="'. $this->tree[$cnt]['parsed_href'].'" '.$target_blank.'>
										<i class="fa '.$faicon.'"></i>
											<span class="menu-title '.$level.'">
												<strong>'.$this->tree[$cnt]['text'].'</strong>
												'.$label.'
											</span>'.$arrow;
											//'<a href="' . $this->tree[$cnt]['parsed_href'] . '"' . $this->tree[$cnt]['parsed_title'] . $this->tree[$cnt]['parsed_target'] . ' class="'.$linkstyle.'">';
					$a_href_close = '</a>';
				
	
			
		
			//$this->_treeMenu[$menu_name] .= ' drop this ->';
		 
			$this->_treeMenu[$menu_name] .= '&nbsp;' . $a_href_open . $a_href_close . "\n";
	...
	$this->_treeMenu[$menu_name] =
	'<table>'.$this->_treeMenu[$menu_name].'</table>';
