...
/**
 * This class represents an event in the htvcenter engine
 *
 * @package htvcenter
 ...
 * @copyright Copyright 2015, HTBase Corp <contact@htbase.com>
 */
class event
{
...
var $crondaily_time = '943939500';
	...
	*/
	...
	function log($name, $time, $priority, $source, $description, $comment, $capabilities, $status, $image_id, $resource_id) {
		...
		// add to syslog
		$syslog_str="htvcenter $source: ($name) $description";
		$syslog_prio="LOG_ERR";
		switch($priority) {
			case 0:
		...
		$syslog_prio = preg_replace('@htvcenter@', 'htvcenter', $syslog_prio);
		$syslog_str = preg_replace('@htvcenter@', 'htvcenter', $syslog_str);


		syslog($syslog_prio, $syslog_str);

		$logfile = '/usr/share/htvcenter/logs/syslog';
		$logcontent = $syslog_prio.$syslog_str."\n" ;
	...
	*/
	...
	function get_list() {
		...
		$event_name_array = htvcenter_db_get_result_double ($query);
	...
	*/
	...
	function trigger_event_hook($action, $id) {
		...
		foreach ($enabled_plugins as $index => $plugin_name) {
			$plugin_event_hook = $this->RootDir."/plugins/".$plugin_name."/htvcenter-".$plugin_name."-event-hook.php";
			if (file_exists($plugin_event_hook)) {
				require_once "$plugin_event_hook";
				$event_hook_function="htvcenter_"."$plugin_name"."_event";
