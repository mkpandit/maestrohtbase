...
function create_clone_iscsi_deployment($cloud_image_id, $image_clone_name, $disk_size) {
	...
	$image_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/iscsi-storage/bin/htvcenter-iscsi-storage snap -n ".$image_location_name." -s ".$image_clone_name." -i ".$image_password." --htvcenter-cmd-mode background";
...
function remove_iscsi_deployment($cloud_image_id) {
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	...
	$image_location=dirname($image_rootdevice);
	$image_location_name=basename($image_location);
	$image_remove_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/iscsi-storage/bin/htvcenter-iscsi-storage remove -n ".$image_location_name." --htvcenter-cmd-mode background";
...
function resize_iscsi_deployment($cloud_image_id, $resize_value) {
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("resize_iscsi_deployment", $_SERVER['REQUEST_TIME'], 2, "iscsi-deployment-cloud-hook", "Resize image is not supported for iscsi-storage!", "", "", 0, 0, 0);
}



// creates a private copy of the volume of an image
function create_private_iscsi_deployment($cloud_image_id, $private_disk, $private_image_name) {
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("create_private_iscsi_deployment", $_SERVER['REQUEST_TIME'], 2, "iscsi-deployment-cloud-hook", "Creating private image is not supported for iscsi-storage!", "", "", 0, 0, 0);
}
