...
var $lang = array();
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {	
				...
				$command     = $this->htvcenter->get('basedir')."/plugins/iscsi-storage/bin/htvcenter-iscsi-storage add -n ".$name." -m ".$size;
				...
				$statfile = $this->htvcenter->get('basedir').'/plugins/iscsi-storage/web/storage/'.$storage->resource_id.'.iscsi.stat';
	...
	//--------------------------------------------
	function set_max() {
		$statfile = $this->htvcenter->get('basedir').'/plugins/iscsi-storage/web/storage/'.$this->resource->id.'.iscsi.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
