...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->htvcenter->get('basedir').'/plugins/iscsi-storage/web/storage/'.$resource->id.'.iscsi.stat';
	}
						...
						if($line !== '') {
							...
							if($i === 0) {
								...
								if ($volume_size == "clone_in_progress") {
									...
									// progressbar
									$t->add(uniqid('b'), 'id');
									$t->add($this->htvcenter->get('baseurl').'/api.php?action=plugin&plugin=iscsi-storage&iscsi_storage_action=progress&name='.$this->resource->id.'.iscsi.'.$name.'.sync_progress', 'url');
