...
var $lang = array(
	'documentation' => array (
		...
		'introduction_content' => 'The "iSCSI-Storage" plugin manages iSCSI-Server in htvcenter.
					   ',
		'requirements_title' => 'Requirements',
		'requirements_list' => 'A resource for the iSCSI-Storage Storage (this can be a remote system integrated into htvcenter e.g. via the "local-server" plugin or the htvcenter server itself)',
	...
	'bootservice' => array (
		...
		'boot_service_content' => 'The iSCSI-Storage Plugin provides an htvcenter Boot-Service.
			This "iSCSI-Storage Boot-Service" is automatically downloaded and executed by the htvcenter-Client on all integrated Systems.
			...
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service view -n iscsi-storage -a [appliance-name]</b></i>
			<br>
			<br>
			...
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service configure -n iscsi-storage -a [appliance-name] -k [key] -v [value]</b></i>
			<br>
			<br>
	...
	function documentation( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new iscsi_storage_about_documentation($this->htvcenter, $this->response);
	...
	function bootservice( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new iscsi_storage_about_bootservice($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
