...
global $event;
	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Authenticating $image_name / $image_rootdevice to resource $resource_mac with password $image_password", "", "", 0, 0, $appliance_id);
				$auth_start_cmd = $htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/".$deployment_plugin_name."/bin/htvcenter-".$deployment_plugin_name." auth -n ".$image_name." -r ".$image_rootdevice." -i ".$image_password." -u ".$htvcenter_admin_user->name." -p ".$htvcenter_admin_user->password." --htvcenter-cmd-mode background";
				...
				if (strlen($install_from_nfs_param)) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $ip_storage_id:$ip_storage_ip:$ip_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if (strlen($transfer_from_nfs_param)) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Transfer-to-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if ($run_disable_deployment_export == 1) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the deployment exports when resource $appliance->resources is fully up.", "", "", 0, 0, $appliance_id);
				}
			...
			case "stop":
				...
				$image_authentication->add($image_auth_ar);
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the root-fs exports when resource $appliance->resources is idle again.", "", "", 0, 0, $appliance_id);
				break;
	...
	*/
	...
	function storage_auth_stop($image_id) {
		...
		$event->log("storage_auth_stop_in_background", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Authenticating $image_name / $image_rootdevice with password $image_password", "", "", 0, 0, $appliance_id);
		$auth_stop_cmd = $htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/".$deployment_plugin_name."/bin/htvcenter-".$deployment_plugin_name." auth -r ".$image_rootdevice." -i ".$image_password." --htvcenter-cmd-mode background";
		$resource = new resource();
	...
	*/
	...
	function storage_auth_deployment_stop($image_id) {
		...
		if (strlen($transfer_from_nfs_param)) {
			...
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-iscsi-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
