...
var $lang = array(
	'login' => array (
		...
		'form_port' => 'VNC Port',
		'info' => 'htvcenter could not determine the VNC port to set up the proxy. Please type in the 2 last bits of the port e.g. 01, 10 or 45 and submit the form.',
...
var $jsurl = '/htvcenter/base/plugins/novnc/novncjs/';
	...
	function login( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new novnc_login($this->htvcenter, $this->response);
	...
	function console( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new novnc_console($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
