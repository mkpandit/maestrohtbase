...
var $lang = array(
	...
	'hooks' => array(
		...
		'dashboard-quicklink-hook' => 'The dashboard-quicklink-hook provides an option for plugins to add a icon and URL to the Quicklink setion on the htvcenter dashboard.',
		...
		'event-hook' => 'The event-hook is triggered when a specific event arrives in htvcenter and allows plugins to run custom actions.',
		...
		'role-hook' => 'The role-hook provides plugins the capability to define custom role-based permissions for every action in htvcenter.',
	),
	'js' => array(
		'tab' => 'JS',
	...
	function __construct($htvcenter, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/chatbot/lang", 'chatbot.ini');
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.select.class.php');
			$controller = new chatbot_select($this->htvcenter, $this->response);
	...
	function rest( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.rest.class.php');
			$controller = new chatbot_rest($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function api( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.api.class.php');
			$controller = new chatbot_api($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function template( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.template.class.php');
			$controller = new chatbot_template($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function lang( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.lang.class.php');
			$controller = new chatbot_lang($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function docblock( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.docblock.class.php');
			$controller = new chatbot_docblock($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function hooks( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.hooks.class.php');
			$controller = new chatbot_hooks($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function js( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.js.class.php');
			$controller = new chatbot_js($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function css( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/chatbot/class/chatbot.css.class.php');
			$controller = new chatbot_css($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function __download() {
		...
		if($this->response->html->request()->get('plugin_name') !== '') {
			...
			$path = $this->htvcenter->get('basedir').'/plugins/'.$dir.'/web/'.$file;
			...
			if($this->response->html->request()->get('base_name') !== 'plugins') {
				...
				$path = $this->htvcenter->get('basedir').'/web/base/server/'.$dir.'/'.$file;
				$mime = $this->response->html->request()->get('mime');
			}
			elseif ($this->response->html->request()->get('base_name') === 'plugins') {
				$dir  = 'aa_plugins';
				$path = $this->htvcenter->get('basedir').'/web/base/plugins/aa_plugins/'.$file;
				$mime = $this->response->html->request()->get('mime');
			}
		}
