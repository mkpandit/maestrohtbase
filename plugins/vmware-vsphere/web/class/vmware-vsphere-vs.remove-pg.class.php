<?php
/**
 * vSphere Hosts remove PortGroup from VSwitch
 *
...
class vmware_vsphere_vs_remove_pg
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-vs-remove-pg.tpl.php');
		...
		if( $portgroup_arr !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($portgroup_arr as $key => $pg) {
					...
					$pg  = str_replace(" ", "@", $pg);
					$command  = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-network remove_pg -i ".$this->resource->ip." -e ".$this->host." -n ".$vs_name." -g ".$pg;
