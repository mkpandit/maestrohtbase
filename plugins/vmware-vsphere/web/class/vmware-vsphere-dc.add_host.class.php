...
class vmware_vsphere_dc_add_host
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-dc-add_host.tpl.php');
	...
	function ds_add_host() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-datacenter addhost -i ".$this->resource->ip." -n ".$this->datacenter;
			$command .= ' -e '.$hostip;
			$command .= ' -u '.$user;
			$command .= ' -p '.$password;
	...
	function get_response() {
		...
		$form = $response->get_form($this->actions_name, 'add_host');
