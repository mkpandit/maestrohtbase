...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-vm-shutdown.tpl.php');
		...
		if( $data !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $vsphere) {
					...
					$command  = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-vm host_shutdown -i ".$vsphere_host_resource->ip;
					$command .= ' --htvcenter-ui-user '.$this->user->name;
					$command .= ' --htvcenter-cmd-mode background';
					$htvcenter_server = new htvcenter_server();
					$htvcenter_server->send_command($command, NULL, true);
					$form->remove($this->identifier_name.'['.$key.']');
					$message[] = sprintf($this->lang['msg_shutdown'], $vsphere_host_appliance->name);
				}
				if(count($errors) === 0) {
