...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-select.tpl.php');
	...
	function select() {
		...
		foreach ($vmware_vsphere_array as $index => $vsphere) {
			...
			$a->css     = 'edit';
			$a->href    = $this->response->get_url($this->actions_name, "vm").'&appliance_id='.$vsphere_appliance_id;
