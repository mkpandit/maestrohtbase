<?php
/**
 * vSphere Hosts remove iSCSI DataStore
 *
...
class vmware_vsphere_ds_remove_iscsi
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-ds-remove-iscsi.tpl.php');
		...
		if( $this->ds_name !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				if(isset($error)) {
					...
					$command     = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-datastore remove_iscsi -i ".$this->resource->ip." -n ".$this->ds_name." -t ".$target." -q ".$ip;
