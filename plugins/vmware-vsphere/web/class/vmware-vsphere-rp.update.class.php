<?php
/**
 * vSphere Hosts ResourcePool Manager
 *
...
class vmware_vsphere_rp_update
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-rp-update.tpl.php');
	...
	function update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command  = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-rp update";
			$command .= " -i ".$this->resource->ip;
			$command .= " -n ".$this->resourcepool;
				...
				if(count($lines) >= 1) {
					foreach($lines as $line) {
						if($line !== '') {
							...
							if($this->resourcepool === $line['name']) {
								$foundrespool=true;
							}
						}
					}
			...
			if (!$foundrespool) {
				$error = sprintf($this->lang['error_exists'], $this->resourcepool);
			}
	...
	function get_response() {
		...
		$d['name']['disabled']						= true;
		$d['name']['validate']['regex']				= '/^[a-z0-9._-]+$/i';
		$d['name']['validate']['errormsg']			= sprintf($this->lang['error_name'], 'a-z0-9._-');
		$d['name']['object']['type']				= 'htmlobject_input';
		$d['name']['object']['attrib']['name']		= 'name';
		$d['name']['object']['attrib']['type']		= 'text';
		$d['name']['object']['attrib']['value']		= $this->resourcepool;
