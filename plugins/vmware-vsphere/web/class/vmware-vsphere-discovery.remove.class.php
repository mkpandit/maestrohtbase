<?php
/**
 * Removes discovered vSphere Hosts
 *
...
class vmware_vsphere_discovery_remove
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-discovery-remove.tpl.php');
	...
	function delete() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$appliance->get_instance_by_name($this->discovery->vmw_vsphere_ad_hostname);
			if ($appliance->id > 0) {
	...
	function get_response() {
		...
		if( $id !== '' ) {
			$this->discovery->get_instance_by_id($id);
			$d['param_f']['label']                       = "IP:".$this->discovery->vmw_vsphere_ad_ip." Mac:".$this->discovery->vmw_vsphere_ad_mac;
