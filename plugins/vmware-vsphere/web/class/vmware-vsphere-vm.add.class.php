...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-vsphere-vm-add.tpl.php');
	...
	function vm_add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$rspool			= $form->get_request('resourcepool');
			$datacenter		= $form->get_request('datacenter');
			$vmxversion	= $form->get_request('vmxversion');
			$disktype		= $form->get_request('disktype');
			...
			if ($form->get_request('existing_vmdk') !== '') {
				...
				$disksize = intval($form->get_request('disk'));
				$disk_parameter = " -d ".$disksize." --disktype ".$disktype;
			} else {
			...
			if(!$form->get_errors()) {
				...
				if(isset($error)) {
					...
					// set resource type
					$vmtype = $this->response->html->request()->get('vmtype');
					if($vmtype === 'vmware-vsphere-vm-net') {
						$virtualization = new virtualization();
						$virtualization->get_instance_by_type("vmware-vsphere-vm-net");
					} else {
						$virtualization = new virtualization();
					...
					$next_vnc_port = $vm_resource->generate_vnc_port($this->resource->id);
					$next_vnc_port_cmd = $next_vnc_port + 5900;
					$resource_fields["resource_vnc"] = $next_vnc_port;
					...
					$command  = $this->htvcenter->get('basedir')."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-vm create";
					$command .= " -i ".$this->resource->ip;
					$command .= " -n ".$name;
					$command .= " --resourcepool ".$rspool;
					$command .= " --vmxversion ".$vmxversion;
							...
							foreach($lines as $line) {
								if($line !== '') {
									...
									if($name === $line['name']) {
										...
										$resource_vnc_fields["resource_vnc"] = $vm_configuration['hostip'].":".$vncport;
					...
					if($vmtype === 'vmware-vsphere-vm-local') {
						...
						if ($found_vmware_vsphere) {
							...
							if ($vm_using_existing_vmdk) {
								...
								$image_fields['image_capabilities'] = 'TYPE='.$guest_id;
								if (isset($disksize)) {
									$image_fields['image_size'] = $disksize;
								}
	...
	function get_response() {
					...
					if($line !== '') {
						...
						switch ($line['t']) {
							...
							case 'rs':
								...
								break;
							case 'dc':
								$datacenter_select_arr[] = array($line['name'],$line['name']);
								break;
						}
					}
				}
		...
		$image = new image();
		$vmdk_image_id_ar = $image->get_ids_by_type('vsphere-deployment');
		...
		$vm_mac = $this->verify_vmware_vsphere_mac_add($vm_resource->mac);
		// 1 nic
		$vm_resource->generate_mac();
		$vm_mac1 = $this->verify_vmware_vsphere_mac_add($vm_resource->mac);
		// 2 nic
		$vm_resource->generate_mac();
		$vm_mac2 = $this->verify_vmware_vsphere_mac_add($vm_resource->mac);
		// 3 nic
		$vm_resource->generate_mac();
		$vm_mac3 = $this->verify_vmware_vsphere_mac_add($vm_resource->mac);
		// 4 nic
		$vm_resource->generate_mac();
		$vm_mac4 = $this->verify_vmware_vsphere_mac_add($vm_resource->mac);
		...
		$d['datacenter']['label']							= $this->lang['form_datacenter'];
		$d['datacenter']['object']['type']					= 'htmlobject_select';
		$d['datacenter']['object']['attrib']['index']		= array(0,1);
		$d['datacenter']['object']['attrib']['id']			= 'datacenter';
		$d['datacenter']['object']['attrib']['name']		= 'datacenter';
		$d['datacenter']['object']['attrib']['options']		= $datacenter_select_arr;
		...
		$d['vmxversion']['label']							= $this->lang['form_vmxversion'];
		$d['vmxversion']['object']['type']					= 'htmlobject_select';
		$d['vmxversion']['object']['attrib']['index']		= array(0,1);
		$d['vmxversion']['object']['attrib']['id']			= 'vmxversion';
		$d['vmxversion']['object']['attrib']['name']		= 'vmxversion';
		$d['vmxversion']['object']['attrib']['options']		= $vmxversion_select_arr;
		$d['vmxversion']['object']['attrib']['selectred']	= array('vmx-07');
		...
		$d['disktype']['label']							= $this->lang['form_disktype'];
		$d['disktype']['object']['type']				= 'htmlobject_select';
		$d['disktype']['object']['attrib']['index']		= array(0,1);
		$d['disktype']['object']['attrib']['id']		= 'disktype';
		$d['disktype']['object']['attrib']['name']		= 'disktype';
		$d['disktype']['object']['attrib']['options']	= $disktype_select_arr;
		...
		$gids = $this->file->get_contents($this->htvcenter->get('basedir').'/plugins/vmware-vsphere/conf/vmware-guest-ids.csv');
		$gids = explode("\n",$gids);
		$guestids = array();
	 ...
	 */
	//--------------------------------------------
	function verify_vmware_vsphere_mac_add($mac) {
