...
class vmware_vsphere_host_controller
{
/**
...
var $lang = array(
	...
	'edit' => array (
		...
		'action_add_local_host' => 'Add local VM',
		'action_add_network_host' => 'Add network VM',
	...
	'add' => array (
		...
		'action_add_host_image' => 'Add a new VM Image',
	...
	'remove' => array (
		'tab' => 'Remove VM',
		'label' => 'Remove Virtual Machine(s) from ESX Host %s',
		'msg_removed' => 'Removed Virtual Machine %s',
		'msg_host_resource_still_in_use' => 'VM %s resource id %s is still in use by server %s',
	...
	'start' => array (
		'tab' => 'Start VM',
		'label' => 'Start Virtual Machine(s) on ESX Host %s',
	...
	'stop' => array (
		'tab' => 'Stop VM',
		'label' => 'Stop Virtual Machine(s) on ESX Host %s',
	...
	function __construct($htvcenter, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/vmware-vsphere/lang", 'vmware-vsphere-host.ini');
	...
	//--------------------------------------------
	function api() {
		require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.api.class.php');
		$controller = new vmware_vsphere_host_api($this);
		$controller->action();
	}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.select.class.php');
				$controller = new vmware_vsphere_host_select($this->htvcenter, $this->response, $this);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.edit.class.php');
				$controller                  = new vmware_vsphere_host_edit($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			if($this->__reload_host_components()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.add.class.php');
				$controller                  = new vmware_vsphere_host_add($this->htvcenter, $this->response, $this);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function update( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host_components()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.update.class.php');
				$controller                  = new vmware_vsphere_host_update($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function clonevm( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host_components()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.clone.class.php');
				$controller                  = new vmware_vsphere_host_clone($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.remove.class.php');
				$controller                  = new vmware_vsphere_host_remove($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function start( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.start.class.php');
				$controller                  = new vmware_vsphere_host_start($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function stop( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.stop.class.php');
				$controller                  = new vmware_vsphere_host_stop($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function import( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.import.class.php');
			$controller                  = new vmware_vsphere_host_import($this->htvcenter, $this->response, $this);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function boot( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_host()) {
				require_once($this->rootdir.'/plugins/vmware-vsphere/class/vmware-vsphere-host.boot.class.php');
				$controller                  = new vmware_vsphere_host_boot($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	 ...
	 */
	//--------------------------------------------
	function __reload_host() {
		$appliance_id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		$appliance->get_instance_by_id($appliance_id);
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-host post_host_list -i ".$resource->ip;
		$command .= ' --htvcenter-ui-user '.$this->user->name;
		$command .= ' --htvcenter-cmd-mode background';
		$file = $this->rootdir.'/plugins/vmware-vsphere/vmware-vsphere-stat/'.$resource->ip.'.host_list';
		if(file_exists($file)) {
			unlink($file);
		}
	 ...
	 */
	//--------------------------------------------
	function __reload_host_components() {
		$appliance_id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		$appliance->get_instance_by_id($appliance_id);
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/vmware-vsphere/bin/htvcenter-vmware-vsphere-host post_host_components -i ".$resource->ip;
		$command .= ' --htvcenter-ui-user '.$this->user->name;
		$command .= ' --htvcenter-cmd-mode background';
		$file = $this->rootdir.'/plugins/vmware-vsphere/vmware-vsphere-stat/'.$resource->ip.'.host_components';
		if(file_exists($file)) {
			unlink($file);
		}
		$htvcenter_server = new htvcenter_server();
