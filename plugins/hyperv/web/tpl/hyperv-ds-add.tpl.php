...
<script type="text/javascript">
	...
	var filepicker = {
		init : function() {
			...
			$.ajax({
				url: "{baseurl}/api.php?action=plugin&plugin=hyperv&controller=hyperv-vm&appliance_id={appliance_id}&path=C:/&{actions_name}=filebrowser",
				dataType: "text",
		...
		browse : function(target) {
			document.getElementById('canvas').innerHTML = '<img src="{baseurl}/img/loading.gif" style="margin-top:150px;">';
			$.ajax({
				url: "{baseurl}/api.php?action=plugin&plugin=hyperv&controller=hyperv-vm&appliance_id={appliance_id}&path="+target+"&{actions_name}=filebrowser",
				dataType: "text",
				success: function(response) {
					document.getElementById('canvas').innerHTML = response;
				}
