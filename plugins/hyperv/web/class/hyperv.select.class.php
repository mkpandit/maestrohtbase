...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/hyperv-select.tpl.php');
	...
	function select() {
		...
		$table = $this->response->html->tablebuilder('hyperv_select', $this->response->get_array($this->actions_name, 'select'));
		...
		foreach ($hyperv_array as $index => $hyperv) {
			...
			$a->css     = 'edit';
			$a->href    = $this->response->get_url($this->actions_name, "vm").'&appliance_id='.$hyperv_appliance_id;
