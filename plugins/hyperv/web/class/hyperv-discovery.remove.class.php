<?php
/**
 * Removes discovered Hyper-V Hosts
 *
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/hyperv-discovery-remove.tpl.php');
	...
	function delete() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$appliance->get_instance_by_name($this->discovery->hyperv_ad_hostname);
			if ($appliance->id > 0) {
	...
	function get_response() {
		...
		if( $id !== '' ) {
			$this->discovery->get_instance_by_id($id);
			$d['param_f']['label']                       = "IP:".$this->discovery->hyperv_ad_ip." Mac:".$this->discovery->hyperv_ad_mac;
