...
var $lang = array();
	...
	function action() {
		...
		$data = $this->update();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/hyperv-vs-update.tpl.php');
	...
	function update() {
		...
		if($this->virtualization->type === 'hyperv') {
			...
			$table = $this->response->html->tablebuilder('hyperv_pg_list', $this->response->get_array($this->actions_name, 'update'));
			// keep the vs_name
			$table->sort            = 'name';
			$table->limit           = 10;
			$table->offset          = 0;
