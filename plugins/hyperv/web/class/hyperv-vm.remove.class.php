...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/hyperv-vm-remove.tpl.php');
	...
	function vm_remove() {
		...
		if( $data !== '' ) {
			...
			foreach($data as $ex) {
				...
				$d['param_f'.$i]['object']['attrib']['value']   = str_replace(' ', '@',$ex);
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $vm) {
					$command  = $this->htvcenter->get('basedir')."/plugins/hyperv/bin/htvcenter-hyperv-vm remove -i ".$this->resource->ip." -n ".$vm;
					$command .= ' --htvcenter-ui-user '.$this->user->name;
					$command .= ' --htvcenter-cmd-mode fork';

					// first find the resource to remove 
					$vm_resource = new resource();
					$vm_resource->get_instance_id_by_hostname($vm);
					$vm_resource->get_instance_by_id($vm_resource->id);
					# end remove loop
					$appliance    = new appliance();
