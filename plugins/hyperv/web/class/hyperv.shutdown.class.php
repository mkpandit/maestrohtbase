...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/hyperv-vm-shutdown.tpl.php');
		...
		if( $data !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $hyperv) {
					...
					$command  = $this->htvcenter->get('basedir')."/plugins/hyperv/bin/htvcenter-hyperv-vm host_shutdown -i ".$hyperv_host_resource->ip;
					$command .= ' --htvcenter-ui-user '.$this->user->name;
					$command .= ' --htvcenter-cmd-mode fork';
					$htvcenter_server = new htvcenter_server();
					$htvcenter_server->send_command($command, NULL, true);
					$form->remove($this->identifier_name.'['.$key.']');
					$message[] = sprintf($this->lang['msg_shutdown'], $hyperv_host_appliance->name);
				}
				if(count($errors) === 0) {
