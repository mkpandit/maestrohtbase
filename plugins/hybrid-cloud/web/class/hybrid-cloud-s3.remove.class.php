...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/hybrid-cloud-s3-remove.tpl.php');
	...
	function remove() {
		...
		$bucket_name  = $response->html->request()->get('bucket_name');
		$form     = $response->form;
		...
		$d['param_f']['label']                       = $bucket_name;
		$d['param_f']['object']['type']              = 'htmlobject_input';
		$d['param_f']['object']['attrib']['type']    = 'checkbox';
		$d['param_f']['object']['attrib']['name']    = 'bucket_name';
		$d['param_f']['object']['attrib']['value']   = $bucket_name;
		...
		if(!$form->get_errors() && $response->submit()) {
			$errors = array();
			$message = array();
			$this->s3->deleteBucket($bucket_name);
			$message[] = sprintf($this->lang['msg_removed'], $bucket_name);
