...
function find_phys_resource_poweroff($app) {
	...
	$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Trying to find a offline physical resource for appliance $app->name .", "", "", 0, 0, $resource_id);
			...
			if ($restype_id == $appliance_virtualization) {
				...
				if ((strlen($app->cpunumber)) && (strcmp($app->cpunumber, "0"))) {
					if (strcmp($app->cpunumber, $resource->cpunumber)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong CPU-number, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				// cpu-speed
				if ((strlen($app->cpuspeed)) && (strcmp($app->cpuspeed, "0"))) {
					if (strcmp($app->cpuspeed, $resource->cpuspeed)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong CPU-speed, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				// cpu-model
				if ((strlen($app->cpumodel)) && (strcmp($app->cpumodel, "0"))) {
					if (strcmp($app->cpumodel, $resource->cpumodel)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong CPU-model, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				// memtotal
				if ((strlen($app->memtotal)) && (strcmp($app->memtotal, "0"))) {
					if (strcmp($app->memtotal, $resource->memtotal)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong amount of Memory, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				// swaptotal
				if ((strlen($app->swaptotal)) && (strcmp($app->swaptotal, "0"))) {
					if (strcmp($app->swaptotal, $resource->swaptotal)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong amount of Swap, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				// nics
				if ((strlen($app->nics)) && (strcmp($app->nics, "0"))) {
					if (strcmp($app->nics, $resource->nics)) {
						$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it has the wrong nic count, skipping.", "", "", 0, 0, $resource_id);
						continue;
					}
				}
				...
				if ($resource_can_start_from_off == 1) {
					$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name with start-from-off enabled.", "", "", 0, 0, $resource_id);
				} else {
					$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name but it cannot start-from-off, skipping.", "", "", 0, 0, $resource_id);
					continue;
				}

				$found_new_resource=1;
				$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found new resource $resource->id for appliance $app->name .", "", "", 0, 0, $resource_id);
				break;
			}
		}
	}
	...
	if ($found_new_resource == 0) {
		$event->log("find_phys_resource_poweroff", $_SERVER['REQUEST_TIME'], 4, "htvcenter-highavailability-ha-hook.php", "Could not find a free resource for appliance $app->name !", "", "", 0, 0, 0);
		return $app;
	}
...
function find_virtualization_host($v_plugin_name, $avoid_resource_id) {
	...
	$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Trying to find a Virtualizatin Host type  $vhost_type->type $vhost_type->name", "", "", 0, 0, 0);
	$ha_conf_file="$htvcenter_SERVER_BASE_DIR/htvcenter/plugins/highavailability/etc/htvcenter-plugin-highavailability.conf";
	$store = htvcenter_parse_conf($xen_vm_conf_file);
	extract($store);
	$migrate_when_failover=$store['htvcenter_HA_MIGRATE_FAILED_VM'];
			...
			if ($appliance->stoptime == 0 || $appliance->resources == 0) {
				...
				if (!strcmp($host_resource->state, "active")) {
					if ($appliance->virtualization == $vhost_type->id) {
						$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "- Found appliance ".$id." as potential candidate for creating the new VM", "", "", 0, 0, 0);
	...
	if (!strcmp($migrate_when_failover, "true")) {
		if ($v_plugin_name != 'hybrid-cloud') {
			$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "- Avoiding origin Host of the VM in error", "", "", 0, 0, 0);
	...
	if (count($active_appliance_list) < 1) {
		$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Warning ! There is no virtualization host type $vhost_type->name available to bring up a new VM", "", "", 0, 0, 0);
		return -1;
	}
	// if we found more than one host remove the origin vhostid of the failed-resource
	if (count($active_appliance_list) > 1) {
		$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "- There is more than once Host candidate for creating the new VM, trying to avoid origin Host.", "", "", 0, 0, 0);
		$active_appliance_list = $active_appliance_list_without_origin_vhost;
	}
	...
	if ($less_load_resource_id >= 0) {
		$event->log("find_virtualization_host", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found Virtualization host resource $less_load_resource_id as the target for the new vm ", "", "", 0, 0, 0);
	}
...
function find_virtualization_host_appliance_poweroff($v_plugin_name, $start_from_off_timeout) {
	...
	$vhost_type = new virtualization();
	$vhost_type->get_instance_by_type($v_plugin_name);
	$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Trying to find a Virtualizatin Host type  $vhost_type->type $vhost_type->name in poweroff state.", "", "", 0, 0, 0);
	// for all in appliance list, find virtualization host appliances
	$appliance_tmp = new appliance();
	$appliance_id_list = $appliance_tmp->get_all_ids();
			...
			if ((!strcmp($appliance->state, "stopped")) && (!strcmp($sfo_resource->state, "off"))) {
				if ($appliance->virtualization == $vhost_type->id) {
					...
					if ($can_start_from_off == 1) {
						...
						$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Adding appliance ".$id." as possible candidate to wakeup.", "", "", 0, 0, 0);
					} else {
						$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.phpp", "Resource ID ".$sfo_resource->id." cannot start-from-off. Skipping.", "", "", 0, 0, 0);
					}
				}
			}
		}
	...
	if (count($powerd_off_appliance_list) < 1) {
		$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "There is no virtualization host type ".$vhost_type->name." available to start-from-off!", "", "", 0, 0, 0);
		return 0;
	}
	...
	$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Waking up Host appliance $in_active_id, waiting until it is fully active ...", "", "", 0, 0, 0);
	...
	$htvcenter_server->send_command("htvcenter_assign_kernel $host_appliance_resource->id $host_appliance_resource->mac $host_appliance_kernel->name");
	sleep(5);
	...
	while (0 == 0) {
		...
		$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Waiting for Host resource id ".$in_active_resource->id." to startup from power-off.", "", "", 0, 0, 0);
		...
		if (!strcmp($in_active_resource->state, "active")) {
			...
			$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Host resource id ".$in_active_resource->id." successfully started from power-off.", "", "", 0, 0, 0);
			return $in_active_id;
		}
		if ($start_from_off_timeout <= $sec_loops) {
			$event->log("find_virtualization_host_appliance_poweroff", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Timeout while waiting for resource id ".$in_active_resource->id." to start-from-off.", "", "", 0, 0, 0);
			return 0;
		}
	}
}
...
// this is the HA hook
function htvcenter_highavailability_ha_hook($resource_id) {
	global $event;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_EXEC_PORT;
	...
	$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Handling error event of resource $resource_id", "", "", 0, 0, $resource_id);
	...
	if ($resource_serves_appliance == 0) {
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Not handling HA for idle Resource $resource_id.", "", "", 0, 0, $resource_id);
		return;
	}
	// is the appliance HA at all ?
	if ($appliance->highavailable <> 1) {
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Appliance $appliance->id is in error but not marked as high-available.", "", "", 0, 0, $resource_id);
		return;
	}

	$ha_appliance_lock_file = "$htvcenter_SERVER_BASE_DIR/htvcenter/plugins/highavailability/lock/ha-lock.".$appliance->id;
	if (file_exists($ha_appliance_lock_file)) {
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "HA for appliance $appliance->id is locked by $ha_appliance_lock_file !", "", "", 0, 0, $resource_id);
		return;
	}

	$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Trying to find a new resource for $appliance->id", "", "", 0, 0, $resource_id);
	...
	if ($appliance->resources == $resource_id) {
		...
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Trying to find a resource to wakeup for appliance $appliance->id !", "", "", 0, 0, $resource_id);
		...
		if ($appliance_virtualization == 1) {
			// physical
			$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Need to find a physical resource for appliance $appliance->id !", "", "", 0, 0, $resource_id);
			$appliance = find_phys_resource_poweroff($appliance);

		} else {
			// if it is a VM maybe we can start one on a Virtualization Host ?
			$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Trying to find a Hypervisor to create a new resource for appliance $appliance->id !", "", "", 0, 0, $resource_id);
			...
			if ($virtualization_host_resource_id < 0) {
				...
				$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "There is no active Virtualization hosts available! Trying to wake-up ...", "", "", 0, 0, 0);
				...
				if ($host_appliance_id_powerd_off > 0) {
					...
					$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found VM Host appliance ID ".$host_appliance_id_powerd_off." Res. ID ".$virtualization_host_resource_id." and woke it up.", "", "", 0, 0, 0);
				} else {
					$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Did not found any powerd-off Virtualization Hosts to wake-up!", "", "", 0, 0, 0);
					return;
				}
			}
			...
			$create_resource_id = (int)str_replace(".", "", str_pad(microtime(true), 15, "0"));
			// create resource in htvcenter
			...
			$htvcenter_server->send_command("htvcenter_server_add_resource $create_resource_id $create_resource_mac $create_resource_ip");
			...
			$create_resource_fields["resource_vname"]=$create_resource_name;
			$create_resource = new resource();
			$create_resource->add($create_resource_fields);

			// plug in the virtualization cloud hook
			$virtualization_ha_hook = "$RootDir/plugins/$virtualization_plugin_name/htvcenter-$virtualization_plugin_name-ha-cmd-hook.php";
			if (file_exists($virtualization_ha_hook)) {
				$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found plugin $virtualization_plugin_name handling to create the VM.", "", "", 0, 0, $create_resource_id);
				require_once "$virtualization_ha_hook";
				$virtualization_method="create_".$virtualization->type;
				$virtualization_method=str_replace("-", "_", $virtualization_method);
				$virtualization_method($virtualization_host_resource_id, $create_resource_name, $create_resource_mac, $create_resource_memory, $create_resource_cpu, $create_resource_swap, $create_resource_additional_nic_str, $resource_id);
				...
				$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Do not know how to create VM from type $virtualization_plugin_name.", "", "", 0, 0, $create_resource_id);
			}
			...
			while (0 == 0) {
				...
				$sec_loops++;
				// check
				$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Waiting for startup of new resource id ".$create_resource_id.".", "", "", 0, 0, 0);
				...
				if ($vm_create_timeout <= $sec_loops) {
					$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Timeout while waiting for new resource id ".$create_resource_id." to start.", "", "", 0, 0, 0);
					return;
				}
			}
	...
	if ($appliance->resources == $resource_id) {
		...
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Could not find or wakeup a free resource for appliance $appliance->id !", "", "", 0, 0, $resource_id);
		return;
	}
	...
	if ($appliance_virtualization == 1) {
		...
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Trying to fence physical resource of appliance $appliance->id !", "", "", 0, 0, $resource_id);
		$plugin = new plugin();
		$enabled_plugins = $plugin->enabled();
		foreach ($enabled_plugins as $index => $plugin_name) {
			$plugin_fence_physical_resource = "$RootDir/plugins/$plugin_name/htvcenter-$plugin_name-resource-fence-hook.php";
			if (file_exists($plugin_fence_physical_resource)) {
				$event->log("fence", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found plugin $plugin_name handling fence-physical-resource event.", "", "", 0, 0, $resource_id);
				require_once "$plugin_fence_physical_resource";
				$resource_fence_function="htvcenter_"."$plugin_name"."_fence_resource";
		...
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Trying to fence the virtual resource of appliance $appliance->id !", "", "", 0, 0, $resource_id);
		...
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Fencing resource $resource_id type $virtualization_plugin_name on host $host_resource->id", "", "", 0, 0, $resource_id);
		...
		// plug in the virtualization ha hook
		$virtualization_ha_hook = "$RootDir/plugins/$virtualization_plugin_name/htvcenter-$virtualization_plugin_name-ha-cmd-hook.php";
		if (file_exists($virtualization_ha_hook)) {
			$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Found plugin $virtualization_plugin_name handling to fence the VM.", "", "", 0, 0, $resource_id);
			require_once "$virtualization_ha_hook";
			$virtualization_method="fence_".$vtype->type;
			$virtualization_method=str_replace("-", "_", $virtualization_method);
			$virtualization_method($host_resource->id, $auto_resource->mac);
		} else {
			$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Do not know how to fence VM from type $virtualization_plugin_name.", "", "", 0, 0, $resource_id);
		}
	}
	...
	$htvcenter_server->send_command("htvcenter_assign_kernel $old_resource->id $old_resource->mac default");
	...
	foreach($ia_id_ar as $ia_list) {
		...
		if ($ia_auth->resource_id == $resource_id) {
			$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 5, "htvcenter-highavailability-ha-hook.php", "Removing image_authentication $ia_auth_id for resource id $resource_id", "", "", 0, 0, $resource_id);
	...
	$htvcenter_server->send_command("htvcenter_assign_kernel $new_resource->id $new_resource->mac $kernel->name");
	...
	if (file_exists($ha_appliance_lock_file)) {
		$event->log("htvcenter_ha_hook", $_SERVER['REQUEST_TIME'], 2, "htvcenter-highavailability-ha-hook.php", "Removing lock for appliance $appliance->id - $ha_appliance_lock_file !", "", "", 0, 0, $resource_id);
