					...
					if (is_array($cs_app_array)) {
						foreach($cs_app_array as $cs_app_id) {
							...
							if (!strcmp($cs_app->state, "active")) {
								...
								$instaname = $cs_app->name;

								$query = "SELECT `size` FROM `cloud_volumes` WHERE `instance_name` = '$instaname'";
								...
								if ($res) {
									while($rez=mysql_fetch_row($res)) {
											...
											$rezo = mysql_fetch_row($reso);
											$ccu = $rezo[0];

											$volccu = $volccu + $ccu;
											$volsize = $volsize + $sizevol;
									}
								}
								...
								if ($volsize != 0) {
									$cccoment = "$disk_cost CCUs for $cr->disk_req MB Disk Space Appliance $cs_app_id (CR $cr->id)";
									$cccoment .= " + $volccu CCUs for additional volumes ( $volsize MB )";
									$disk_cost = $disk_cost + $volccu;
								} else {
									$cccoment = "$disk_cost CCUs for $cr->disk_req MB Disk Space Appliance $cs_app_id (CR $cr->id)";
								}
								//$cccoment = "$disk_cost CCUs for $cr->disk_req MB Disk Space Appliance $cs_app_id (CR $cr->id)";
								$ct->push($cr->id, $cr->cu_id, $disk_cost, $new_cu_ccunits, "Cloud Billing", $cccoment);
								// ha
								if (strlen($cr->ha_req)) {
