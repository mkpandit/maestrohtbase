			...
			if(isset($request) && is_array($request)) {
				...
				foreach($request as $key => $cr_id) {
					...
					if ($remove_cr) {
						...
						$this->cloud_mailer->subject = "htvcenter Cloud: Your request ".$cr_id." has been removed";
						$this->cloud_mailer->template = $this->rootdir."/plugins/cloud/etc/mail/delete_cloud_request.mail.tmpl";
						$arr = array('@@ID@@' => $cr_id, '@@FORENAME@@' => $this->cloud_user->forename, '@@LASTNAME@@' => $this->cloud_user->lastname, '@@CLOUDADMIN@@' => $cc_admin_email);
