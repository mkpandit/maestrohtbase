...
if (!strstr($htvcenter_USER->role, "administrator")) {
	$event->log("AuthenticateSoapUser", $_SERVER['REQUEST_TIME'], 1, "cloud-soap-server.php", "Un-Authorized access to htvcenter SOAP-Service from $htvcenter_USER->name", "", "", 0, 0, 0);
	exit();
}
...
// ######################### cloud user methods ################################
	...
	*/
	...
	function CloudUserCreate($method_parameters) {
		...
		$user_fields['cu_id'] = htvcenter_db_get_free_id('cu_id', $cl_user->_db_table);
	...
	*/
	...
	function CloudUserGroupCreate($method_parameters) {
		...
		$usergroups_fields['cg_id'] = (int)str_replace(".", "", str_pad(microtime(true), 15, "0"));
