...
//		}
		...
		$template = $this->response->html->template($this->tpldir."/cloud-user-enable.tpl.php");
		$template->add($response->form->get_elements());
		$template->add($response->html->thisfile, "thisfile");
		$template->add($this->lang['cloud_user_confirm_enable'], 'confirm_enable');
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(isset($request) && is_array($request)) {
				...
				foreach($request as $key => $cz_u_id) {
					...
					$this->cloud_user->activate_user_status($cz_u_id, 1);
					$message[] = $this->lang['cloud_user_enabled']." - ".$this->cloud_user->name;
				}
				if(count($errors) === 0) {
	...
	function get_response() {
		$toenable = $this->response->html->request()->get($this->identifier_name);
		$response =$this->response;
		$form     = $response->get_form($this->actions_name, 'enable');
		$d        = array();
		if( $toenable !== '' ) {
			$i = 0;
			foreach($toenable as $cz_u_id) {
				$this->cloud_user->get_instance_by_id($cz_u_id);
