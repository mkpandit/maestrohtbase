...
class clouduserlimits {
	...
	*/
	...
	function init() {
		...
		$this->_db_table = "cloud_users_limits";
	...
	function get_instance($id, $cu_id) {
		...
		if ("$id" != "") {
			$clouduserlimits_array = $db->Execute("select * from ".$this->_db_table." where cl_id=$id");
		} else if ("$cu_id" != "") {
			$clouduserlimits_array = $db->Execute("select * from ".$this->_db_table." where cl_cu_id='$cu_id'");
		} else {
	...
	function is_id_free($clouduserlimits_id) {
		$db=htvcenter_get_db_connection();
		$rs = $db->Execute("select cl_id from ".$this->_db_table." where cl_id=$clouduserlimits_id");
		if (!$rs)
			$this->_event->log("is_id_free", $_SERVER['REQUEST_TIME'], 2, "clouduserlimits.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
		else
		if ($rs->EOF) {
			return true;
		} else {
	...
	function add($clouduserlimits_fields) {
		if (!is_array($clouduserlimits_fields)) {
			$this->_event->log("add", $_SERVER['REQUEST_TIME'], 2, "clouduserlimits.class.php", "clouduserlimits_fields not well defined", "", "", 0, 0, 0);
			return 1;
		}
		// set stop time and status to now
		$now=$_SERVER['REQUEST_TIME'];
		$db=htvcenter_get_db_connection();
		$result = $db->AutoExecute($this->_db_table, $clouduserlimits_fields, 'INSERT');
		if (! $result) {
			$this->_event->log("add", $_SERVER['REQUEST_TIME'], 2, "clouduserlimits.class.php", "Failed adding new clouduserlimits to database", "", "", 0, 0, 0);
		}
	}
	...
	function remove($clouduserlimits_id) {
		$db=htvcenter_get_db_connection();
		$rs = $db->Execute("delete from ".$this->_db_table." where cl_id=$clouduserlimits_id");
	}

	// removes clouduserlimits from the database by clouduserlimits_name
	function remove_by_cu_id($cu_id) {
		$db=htvcenter_get_db_connection();
		$rs = $db->Execute("delete from ".$this->_db_table." where cl_cu_id='$cu_id'");
	}
	...
	function get_count() {
		$count=0;
		$db=htvcenter_get_db_connection();
		$rs = $db->Execute("select count(cl_id) as num from ".$this->_db_table);
		if (!$rs) {
			print $db->ErrorMsg();
		} else {
			$count = $rs->fields["num"];
	...
	function get_list() {
		$query = "select cl_id, cl_cu_id from ".$this->_db_table;
		$clouduserlimits_name_array = array();
		$clouduserlimits_name_array = htvcenter_db_get_result_double ($query);
	...
	function get_all_ids() {
		...
		$query = "select cl_id from ".$this->_db_table;
		$db=htvcenter_get_db_connection();
		$rs = $db->Execute($query);
		if (!$rs)
			$this->_event->log("get_list", $_SERVER['REQUEST_TIME'], 2, "clouduserlimits.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
		else
		while (!$rs->EOF) {
