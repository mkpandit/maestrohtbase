<?php
/**
 * Cloud failed-Transaction Controller
 *
...
class cloud_transaction_failed_controller
{
/**
...
var $lang = array(
	...
	function __construct($htvcenter, $response) {
		...
		$this->identifier_name = "cloud_transaction_failed_id";
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/cloud/class/cloud-transaction-failed.select.class.php');
			$controller = new cloud_transaction_failed_select($this->htvcenter, $this->response);
		...
		$content['label']   = $this->lang['cloud_transaction_failed'];
		$content['value']   = $data;
	 ...
	 */
	//--------------------------------------------
	function sync( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/cloud/class/cloud-transaction-failed.sync.class.php');
			$controller = new cloud_transaction_failed_sync($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->identifier_name = $this->identifier_name;
			$controller->message_param = $this->message_param;
