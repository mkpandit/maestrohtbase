...
//		}
		...
		$template = $this->response->html->template($this->tpldir."/cloud-request-unpause.tpl.php");
		$template->add($response->form->get_elements());
		$template->add($response->html->thisfile, "thisfile");
		$template->add($this->lang['cloud_request_confirm_unpause'], 'confirm_unpause');
			...
			if(isset($cr_id)) {
				...
				if ($unpause_cr) {
					if (($this->cloud_request->appliance_id != '') && ($this->cloud_request->appliance_id != 0)) {
						...
						if (($this->cloudappliance->state == 0) && ($this->cloudappliance->cmd == 0)) {
							$unpause_cr=true;
						} else {
							$unpause_cr=false;
						}
				...
				// do we unpause ?
				if ($unpause_cr) {
					// mail user before unpause
					$this->cloud_mailer->to = $this->cloud_user->email;
					$this->cloud_mailer->from = $cc_admin_email;
					$this->cloud_mailer->subject = "htvcenter Cloud: Your request ".$cr_id." has been unpaused";
					$this->cloud_mailer->template = $this->rootdir."/plugins/cloud/etc/mail/unpaused_cloud_appliance.mail.tmpl";
					...
					$message[] = $this->lang['cloud_request_unpaused']." - ".$this->cloud_request->appliance_id;
				} else {
