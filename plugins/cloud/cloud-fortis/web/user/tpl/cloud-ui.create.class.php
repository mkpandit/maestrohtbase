				...
				if (!strcmp($show_ip_mgmt, "true")) {
					if (file_exists($this->rootdir."/plugins/ip-mgmt/.running")) {
						...
						foreach($ip_mgmt_list_per_user as $list) {
							...
							if($ip_mgmt_id == $form->get_request('cloud_ip_select_0')) {
								$dhcpd_ip_add = $list['ip_mgmt_address'];
							}
						}
					}
				}
				//echo "<br>" . $dhcpd_ip_add;
				$mac_add_query = mysql_query("SELECT resource_mac FROM resource_info WHERE resource_vname = '".$form->get_request('cloud_hostname_input')."'");
				$mac_add_data  = mysql_fetch_assoc($mac_add_query);
				$mac_add_d = $mac_add_data['resource_mac'];
				//echo $mac_add_d; //die(); 
				$dhcpd_data = "################\n";
				$dhcpd_data .= $dhcpd_ip_add . "\n";
				$dhcpd_data .= $mac_add_d . "\n";
