...
var filepicker = {
	init : function( element ) {
		...
		$.ajax({
			url: "{baseurl}/api.php?action=plugin&plugin=OCH&controller=OCH-vm&path=/&appliance_id={appliance_id}&{actions_name}=filepicker",
			dataType: "text",
	...
	browse : function(target) {
		document.getElementById('canvas').innerHTML = '<img src="{baseurl}/img/loading.gif" style="margin-top:150px;">';
		$.ajax({
			url: "{baseurl}/api.php?action=plugin&plugin=OCH&controller=OCH-vm&path="+target+"&appliance_id={appliance_id}&{actions_name}=filepicker",
			dataType: "text",
			success: function(response) {
				document.getElementById('canvas').innerHTML = response;	
			}
...
function namegen() {
	...
	document.getElementById('name').value = 'OCH'+name;
}
</script>
...
<div class="tab-content">
	...
	<fieldset>
		<legend>{lang_virtual_disk}</legend>
			<div class="span8">
				<div style="float:left;" class="addleftvm">
					{disk_interface}
				</div>
