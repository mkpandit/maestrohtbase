...
var $lang = array(
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new kvm_vm_select($this->htvcenter, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new kvm_vm_edit($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				...
				$controller                = new kvm_vm_add($this->htvcenter, $this->response, $this);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function update( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				...
				$controller                = new kvm_vm_update($this->htvcenter, $this->response);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new kvm_vm_clone($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function migrate( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_migrate($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function start( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_start($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function stop( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_stop($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reboot( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_reboot($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function iso( $hidden = true ) {
		...
		if( $hidden === true ) {
		    ...
		    $controller                  = new kvm_vm_iso($this->htvcenter, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
	...
	function sysinfo( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new kvm_vm_sysinfo($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload() {
		...
		$command  = $htvcenter_SERVER_BASE_DIR.'/plugins/kvm/bin/htvcenter-kvm-vm post_vm_list';
		...
		$file = $htvcenter_SERVER_BASE_DIR.'/plugins/kvm/web/kvm-stat/'.$resource->id.'.vm_list';
	...
	function reload_bridges() {
		$htvcenter_SERVER_BASE_DIR = $this->htvcenter->get('basedir');
		$command  = $htvcenter_SERVER_BASE_DIR.'/plugins/kvm/bin/htvcenter-kvm-vm post_bridge_config';
		$command .= ' -u '.$this->htvcenter->admin()->name.' -p '.$this->htvcenter->admin()->password;
		$command .= ' --htvcenter-ui-user '.$this->user->name;
		$command .= ' --htvcenter-cmd-mode background';
		$id = $this->response->html->request()->get('appliance_id');
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $htvcenter_SERVER_BASE_DIR.'/plugins/kvm/web/kvm-stat/'.$resource->id.'.bridge_config';
		if($this->file->exists($file)) {
			$this->file->remove($file);
		}
		$resource->send_command($resource->ip, $command);
