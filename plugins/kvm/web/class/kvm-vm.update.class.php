...
var $lang = array();
		...
		if(!isset($respnse->msg)) {
			...
			if(!$form->get_errors() && $this->response->submit()) {
				...
				if(count($errors) > 0 || $form->get_errors()) {
					...
					$command  = $this->htvcenter->get('basedir').'/plugins/kvm/bin/htvcenter-kvm-vm update';
					$command .= ' -n '.$this->vm;
	...
	function get_response() {
		...
		$file = $this->htvcenter->get('basedir').'/plugins/kvm/web/kvm-stat/'.$this->resource->id.'.'.$this->vm.'.vm_config';
		$ini = htvcenter_parse_conf($file);

		$bridges = array();
		$bridge_list = $ini['htvcenter_KVM_BRIDGES'];
		...
		if(isset($bridge_array[0]) && $bridge_array[0] !== '') {
			...
			$this->response->params['mac'] = $ini['htvcenter_KVM_VM_MAC_1'];
			...
			$d['cpus']['object']['attrib']['selected']  = array($ini['htvcenter_KVM_VM_CPUS']);
			...
			if($ini['htvcenter_KVM_VM_RAM'] !== '') {
				$d['memory']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_RAM']);
			}
			...
			if($vmtype === 'kvm-vm-net') {
				...
				if($ini['htvcenter_KVM_VM_CDROM2'] !== '') {
					$cdrompath = str_replace('-drive file=', '',$ini['htvcenter_KVM_VM_CDROM2']);
			...
			$d['disk_interface']['object']['attrib']['selected']= array($ini['htvcenter_KVM_VM_DISK_INTERFACE']);
			...
			$d['mac']['object']['attrib']['value']     = $ini['htvcenter_KVM_VM_MAC_1'];
			...
			$d['bridge']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_BRIDGE_1']);
			...
			$d['nic']['object']['attrib']['selected']  = array($ini['htvcenter_KVM_VM_NIC_TYPE_1']);

			// net 1
			$checked = false;
			if($ini['htvcenter_KVM_VM_MAC_2'] !== '' ) {
				$mac = $ini['htvcenter_KVM_VM_MAC_2'];
				$checked = true;
			} else {
			...
			if(isset($ini['htvcenter_KVM_VM_BRIDGE_2']) && $checked === true) {
				$d['bridge1']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_BRIDGE_2']);
			}
			...
			if($checked === true) {
				$d['nic1']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_NIC_TYPE_2']);
			}

			// net 2
			$checked = false;
			if($ini['htvcenter_KVM_VM_MAC_3'] !== '' ) {
				$mac = $ini['htvcenter_KVM_VM_MAC_3'];
				$checked = true;
			} else {
				$this->resource->generate_mac();
				$mac = $this->resource->mac;
			...
			if(isset($ini['htvcenter_KVM_VM_BRIDGE_3']) && $checked === true) {
				$d['bridge2']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_BRIDGE_3']);
			}
			...
			if($checked === true) {
				$d['nic2']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_NIC_TYPE_3']);
			}

			// net 3
			$checked = false;
			if($ini['htvcenter_KVM_VM_MAC_4'] !== '' ) {
				$mac = $ini['htvcenter_KVM_VM_MAC_4'];
				$checked = true;
			} else {
				$this->resource->generate_mac();
				$mac = $this->resource->mac;
			...
			if(isset($ini['htvcenter_KVM_VM_BRIDGE_4']) && $checked === true) {
				$d['bridge3']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_BRIDGE_4']);
			}
			...
			if($checked === true) {
				$d['nic3']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_NIC_TYPE_4']);
			}

			// net 4
			$checked = false;
			if($ini['htvcenter_KVM_VM_MAC_5'] !== '' ) {
				$mac = $ini['htvcenter_KVM_VM_MAC_5'];
				$checked = true;
			} else {
				$this->resource->generate_mac();
				$mac = $this->resource->mac;
			...
			if(isset($ini['htvcenter_KVM_VM_BRIDGE_5']) && $checked === true) {
				$d['bridge4']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_BRIDGE_5']);
			}
			...
			if($checked === true) {
				$d['nic4']['object']['attrib']['selected'] = array($ini['htvcenter_KVM_VM_NIC_TYPE_5']);
			}
			...
			if($vmtype !== 'kvm-vm-net') {
				...
				if($ini['htvcenter_KVM_VM_BOOT'] === 'cdrom') {
				...
				if(strpos($ini['htvcenter_KVM_VM_BOOT'], 'iso') !== false) {
					$isopath = str_replace('iso:', '',$ini['htvcenter_KVM_VM_BOOT']); 
				...
				if($ini['htvcenter_KVM_VM_BOOT'] === 'local') {
			...
			if($ini['htvcenter_KVM_VM_BOOT'] === 'network') {
			...
			$d['vnc']['object']['attrib']['value']     = $ini['htvcenter_KVM_VM_VNCPASSWORD'];
			...
			$d['vnc_1']['object']['attrib']['value']     = $ini['htvcenter_KVM_VM_VNCPASSWORD'];
			...
			$d['vnc_keymap']['object']['attrib']['selected']= array($ini['htvcenter_KVM_VM_VNCKEYMAP']);
	...
	//--------------------------------------------
	function reload() {
		$command  = $this->htvcenter->get('basedir').'/plugins/kvm/bin/htvcenter-kvm-vm post_vm_config';
		$command .=  ' -n '.$this->vm;
		$command .= ' -u '.$this->htvcenter->admin()->name.' -p '.$this->htvcenter->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $this->htvcenter->get('basedir').'/plugins/kvm/web/kvm-stat/'.$resource->id.'.'.$this->vm.'.vm_config';
