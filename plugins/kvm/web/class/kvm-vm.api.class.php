...
class kvm_vm_api
{
	...
	function filepicker() {
		...
		if ($iso_path !== '') {
			...
			$command  = $this->controller->htvcenter->get('basedir')."/plugins/kvm/bin/htvcenter-kvm-vm iso";
			$command .= ' -q '.$iso_path;
			$command .= ' -u '.$this->admin->name.' -p '.$this->admin->password;
			$command .= ' --htvcenter-ui-user '.$this->user->name;
			$command .= ' --htvcenter-cmd-mode background';

			$file = $this->controller->htvcenter->get('webdir').'/plugins/kvm/kvm-stat/'.$this->resource->id.'.pick_iso_config';
	...
	function progress() {
		...
		$file = $this->controller->htvcenter->get('basedir').'/plugins/kvm/web/kvm-stat/'.$name;
		if($this->file->exists($file)) {
