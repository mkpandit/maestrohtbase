...
var $lang = array();
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command  = $this->htvcenter->get('basedir').'/plugins/kvm/bin/htvcenter-kvm clone';
			...
			if(isset($error)) {
				...
				$file = $this->htvcenter->get('basedir').'/plugins/kvm/web/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress';
				if($this->file->exists($file)) {
					$this->file->remove($file);
				}
				$root_device_ident = $this->htvcenter->get('basedir').'/plugins/kvm/web/storage/'.$this->resource->id.'.'.$name.'.root_device';
