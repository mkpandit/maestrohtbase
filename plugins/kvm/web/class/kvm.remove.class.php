			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->htvcenter->get('basedir').'/plugins/kvm/bin/htvcenter-kvm remove';
						$command .= ' -n '.$lvol;
						...
						$file = $this->htvcenter->get('basedir').'/plugins/kvm/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
