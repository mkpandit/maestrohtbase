...
function kvm_gluster_deployment_wait_for_identfile($sfile) {
	$refresh_delay=1;
	$refresh_loop_max=20;
	$refresh_loop=0;
...
function get_kvm_gluster_deployment_image_rootdevice_identifier($kvm_gluster_id) {
	global $htvcenter_SERVER_BASE_DIR;
	...
	$storage = new storage();
	$storage->get_instance_by_id($kvm_gluster_id);
	// get deployment type
	...
	if (!kvm_gluster_deployment_wait_for_identfile($ident_file)) {
...
function get_kvm_gluster_deployment_image_default_rootfs() {
	return "local";
}

function get_kvm_gluster_deployment_rootfs_transfer_methods() {
	return false;
}

function get_kvm_gluster_deployment_rootfs_set_password_method() {
	return true;
}

function get_kvm_gluster_deployment_is_network_deployment() {
	return false;
}

function get_kvm_gluster_deployment_local_deployment_enabled() {
	return true;
}
