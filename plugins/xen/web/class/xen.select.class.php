...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/xen-select.tpl.php');
	...
	function select() {
		...
		if(count($storages) >= 1) {
			...
			$table = $this->response->html->tablebuilder('xen', $this->response->get_array($this->actions_name, 'select'));
