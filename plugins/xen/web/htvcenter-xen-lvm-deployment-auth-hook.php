	...
	*/
	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		if ($vm_host_resource->id != $storage_resource->id) {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-lvm-deployment-auth-hook.php", "Appliance ".$appliance_id." image IS NOT available on this xen host, ".$storage_resource->id." not equal ".$vm_host_resource->id." !! Assuming SAN Backend", "", "", 0, 0, $appliance_id);
		} else {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-lvm-deployment-auth-hook.php", "Appliance ".$appliance_id." image IS available on this xen host, ".$storage_resource->id." equal ".$vm_host_resource->id.".", "", "", 0, 0, $appliance_id);
		}
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-kvm-xen-deployment-auth-hook.php", "Authenticating ".$image_name." / ".$image_location_name." to resource ".$resource_mac.".", "", "", 0, 0, $appliance_id);
