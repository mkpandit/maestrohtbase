...
// creates a vm
function create_ha_xen_vm($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str, $vm_type) {
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	...
	$vm_create_cmd = "$htvcenter_SERVER_BASE_DIR/htvcenter/plugins/xen/bin/htvcenter-xen-vm create -n ".$name." -m ".$mac." -r ".$memory." -c ".$cpu." -b local ".$additional_nic_str." --htvcenter-cmd-mode background";
...
// fences a vm
function fence_ha_xen_vm($host_resource_id, $mac, $vm_type) {
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	...
	$vm_fence_cmd = "$htvcenter_SERVER_BASE_DIR/htvcenter/plugins/xen/bin/htvcenter-xen-vm fence -m ".$mac." --htvcenter-cmd-mode background";
...
function create_xen_vm_local($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str, $origin_resource_id) {
	global $event;
	create_ha_xen_vm($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str, "xen-vm-local");
}

function create_xen_vm_net($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str, $origin_resource_id) {
	global $event;
	create_ha_xen_vm($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str, "xen-vm-net");
}

function fence_xen_vm_local($host_resource_id, $mac) {
	global $event;
	fence_ha_xen_vm($host_resource_id, $mac, "xen-vm-local");
}

function fence_xen_vm_net($host_resource_id, $mac) {
	global $event;
	fence_ha_xen_vm($host_resource_id, $mac, "xen-vm-net");
}
