...
function htvcenter_xen_cloud_product($cmd, $cloud_hook_config) {
	global $event;
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	...
	$event->log("htvcenter_xen_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-cloud-product-hook.php", "Handling ".$cmd." event", "", "", 0, 0, 0);
	switch($cmd) {
			case "add":
				$event->log("htvcenter_xen_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-cloud-product-hook.php", "Handling cloud-product ".$cmd." event", "", "", 0, 0, 0);
				...
				foreach ($virtualization_id_ar as $key => $value) {
					...
					$pos = strpos($virtualization->type, 'xen-vm-');
					if ($pos !== false) {
						$event->log("htvcenter_xen_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-cloud-product-hook.php", "Adding ".$virtualization->type." as cloud-product", "", "", 0, 0, 0);
						...
						if (!in_array($virtualization_plugin_name, $checked_virtualization_plugins)) {
							...
							foreach ($deployment_id_ar as $key => $value) {
								...
								foreach ($image_id_deployment_ar as $iid_ar) {
									...
									$event->log("htvcenter_xen_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-cloud-product-hook.php", "Adding Image ".$image_id." as private image", "", "", 0, 0, 0);
			...
			case "remove":
				...
				if (file_exists($cloud_product_class)) {
					...
					$event->log("htvcenter_xen_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-cloud-product-hook.php", "Removing resource type ".$virtualization->type." from cloud-products", "", "", 0, 0, 0);
