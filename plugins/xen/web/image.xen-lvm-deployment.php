...
function xen_lvm_deployment_wait_for_identfile($sfile) {
	$refresh_delay=1;
	$refresh_loop_max=20;
	$refresh_loop=0;
...
function get_xen_lvm_deployment_image_rootdevice_identifier($lvm_nfs_id) {
	global $htvcenter_SERVER_BASE_DIR;
	...
	$storage = new storage();
	$storage->get_instance_by_id($lvm_nfs_id);
	// get deployment type
	...
	// send command
	$resource_command=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/xen/bin/htvcenter-xen post_identifier -u ".$htvcenter_ADMIN->name." -p ".$htvcenter_ADMIN->password." -t ".$deployment->type." --htvcenter-cmd-mode background";
	$storage_resource->send_command($storage_resource->ip, $resource_command);
	if (!xen_lvm_deployment_wait_for_identfile($ident_file)) {
...
function get_xen_lvm_deployment_image_default_rootfs() {
	return "local";
}

function get_xen_lvm_deployment_rootfs_transfer_methods() {
	return false;
}

function get_xen_lvm_deployment_rootfs_set_password_method() {
	return true;
}

function get_xen_lvm_deployment_is_network_deployment() {
	return false;
}

function get_xen_lvm_deployment_local_deployment_enabled() {
	return false;
}

function get_xen_lvm_deployment_is_resizable() {
	return true;
}
