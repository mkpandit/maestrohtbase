...
global $event;
	...
	*/
	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		// This enables using a e.g. a NAS or Glustefs backend as shared VM location for all xen hosts
		...
		if ($vm_host_resource->id != $storage_resource->id) {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-bf-deployment-auth-hook.php", "Appliance ".$appliance_id." image IS NOT available on this xen host, ".$storage_resource->id." not equal ".$vm_host_resource->id." !! Assuming SAN Backend", "", "", 0, 0, $appliance_id);
		} else {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-bf-deployment-auth-hook.php", "Appliance ".$appliance_id." image IS available on this xen host, ".$storage_resource->id." equal ".$vm_host_resource->id.".", "", "", 0, 0, $appliance_id);
		}
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-xen-xen-deployment-auth-hook.php", "Authenticating ".$image_name." / ".$image_location_name." to resource ".$resource_mac.".", "", "", 0, 0, $appliance_id);
				$auth_start_cmd = $htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/".$deployment_plugin_name."/bin/htvcenter-".$deployment_plugin_name." auth -n ".$image_name." -r ".$image_rootdevice." -i ".$image_name." -u ".$htvcenter_admin_user->name." -p ".$htvcenter_admin_user->password." -t ".$deployment->type." --htvcenter-cmd-mode background";
