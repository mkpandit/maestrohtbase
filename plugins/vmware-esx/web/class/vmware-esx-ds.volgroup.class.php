...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->rootdir.'/plugins/vmware-esx/vmware-esx-stat/'.$resource->ip.'.vmdk_list';
	}
	...
	function action() {
		...
		$data = $this->ds();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/vmware-esx-ds-volgroup.tpl.php');
	...
	function __reload_vmdks($volgroup) {
		$command  = $this->htvcenter->get('basedir')."/plugins/vmware-esx/bin/htvcenter-vmware-esx-datastore post_vmdk_list -i ".$this->resource->ip." -n ".$this->volgroup;
		$file = $this->rootdir.'/plugins/vmware-esx/vmware-esx-stat/'.$this->resource->ip.'.vmdk_list';
		if(file_exists($file)) {
			unlink($file);
		}
