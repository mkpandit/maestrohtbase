...
class vmware_esx_ds_controller
{
/**
...
var $lang = array(
	...
	function __construct($htvcenter, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/vmware-esx/lang", 'vmware-esx-ds.ini');
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_ds()) {
				require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-ds.edit.class.php');
				$controller                  = new vmware_esx_ds_edit($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_add_nas($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function add_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_add_iscsi($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function remove_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_remove_nas($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function remove_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->__reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_remove_iscsi($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function volgroup( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-ds.volgroup.class.php');
			$controller                  = new vmware_esx_ds_volgroup($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-ds.remove.class.php');
			$controller                  = new vmware_esx_ds_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_ds()) {
				require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-ds.clone.class.php');
				$controller                  = new vmware_esx_ds_clone($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function __reload_ds() {
		...
		$command  = $this->basedir."/plugins/vmware-esx/bin/htvcenter-vmware-esx-datastore post_ds_list -i ".$resource->ip;
