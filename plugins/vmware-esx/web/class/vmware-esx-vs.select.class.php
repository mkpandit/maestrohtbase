...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/vmware-esx-vs-select.tpl.php');
	...
	function select() {
		...
		$table = $this->response->html->tablebuilder('vmware_vs_select', $this->response->get_array($this->actions_name, 'select'));
		...
		foreach ($vmware_esx_array as $index => $esx) {
			...
			$a->css     = 'edit';
			$a->href    = $this->response->get_url($this->actions_name, "edit").'&appliance_id='.$esx_appliance_id;
