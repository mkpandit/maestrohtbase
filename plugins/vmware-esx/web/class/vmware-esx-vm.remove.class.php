		...
		if( $data !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $vm) {
					$command  = $this->htvcenter->get('basedir')."/plugins/vmware-esx/bin/htvcenter-vmware-esx-vm remove -i ".$this->resource->ip." -n ".$vm;
					...
					if (count($appliances_using_resource) > 0) {
						...
						// for vmware-esx-vm-local we have to also remove the image object
						$vm_virtualization = new virtualization();
						$vm_virtualization->get_instance_by_type("vmware-esx-vm-local");
