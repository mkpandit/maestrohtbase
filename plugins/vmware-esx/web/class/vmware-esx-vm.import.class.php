...
var $lang = array();
	...
	function init() {
		...
		#$this->statfile_host = $this->rootdir.'/plugins/vmware-esx/vmware-esx-stat/'.$resource->ip.'.host_statistics';
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-esx-vm-import.tpl.php');
	...
	function vm_import() {
		...
		if(!$form->get_errors() && $response->submit()) {
			...
			$file = $this->rootdir.'/plugins/vmware-esx/vmware-esx-stat/'.$this->resource->ip.'.vm_list';
			// read stats file
			$line_nubmer = 1;
									...
									if (($vm_boot == "local") || ($vm_boot == "cdrom")) {
										...
										if ($image->id == '') {
											...
											if ((isset($vm_appl_exist_check->id)) && ($vm_appl_exist_check->id > 0)) {
												...
												$afields['appliance_comment'] = 'VMWare ESX VM Appliance for Resource '.$id;
