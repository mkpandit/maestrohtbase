...
class vmware_esx_vm_controller
{
/**
...
var $lang = array(
	...
	function __construct($htvcenter, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/vmware-esx/lang", 'vmware-esx-vm.ini');
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm()) {
				require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-vm.edit.class.php');
				$controller                  = new vmware_esx_vm_edit($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm_components()) {
				...
				$controller                  = new vmware_esx_vm_add($this->htvcenter, $this->response, $this);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function update( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm_components()) {
				...
				$controller                  = new vmware_esx_vm_update($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_remove($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function start( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_start($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function stop( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_stop($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function import( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/vmware-esx/class/vmware-esx-vm.import.class.php');
			$controller                  = new vmware_esx_vm_import($this->htvcenter, $this->response, $this);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function boot( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->__reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_boot($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function __reload_vm() {
		...
		$command  = $this->basedir."/plugins/vmware-esx/bin/htvcenter-vmware-esx-vm post_vm_list -i ".$resource->ip;
	...
	function __reload_vm_components() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/vmware-esx/bin/htvcenter-vmware-esx-vm post_vm_components -i ".$resource->ip;
		$command .= ' --htvcenter-ui-user '.$this->user->name;
		$command .= ' --htvcenter-cmd-mode background';
		$file = $this->rootdir.'/plugins/vmware-esx/vmware-esx-stat/'.$resource->ip.'.vm_components';
		if(file_exists($file)) {
			unlink($file);
		}
		$htvcenter_server = new htvcenter_server();
