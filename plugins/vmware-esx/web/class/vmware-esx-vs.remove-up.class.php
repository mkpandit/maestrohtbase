...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/vmware-esx-vs-remove-up.tpl.php');
		...
		if( $uplink !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				if ($vs_name === 'vSwitch0') {
					...
					// handle command
					if($pg_name !== '') {
						$command  = $this->htvcenter->get('basedir')."/plugins/vmware-esx/bin/htvcenter-vmware-esx-network remove_pg_up";
						$command .= " -i ".$this->resource->ip;
						$command .= " -n ".$vs_name;
						$command .= " -g ".$pg_name;
						$command .= " -u ".$uplink;
					} else {
						$command  = $this->htvcenter->get('basedir')."/plugins/vmware-esx/bin/htvcenter-vmware-esx-network remove_vs_up";
						$command .= " -i ".$this->resource->ip;
						$command .= " -n ".$vs_name;
						$command .= " -u ".$uplink;
					}
