...
function htvcenter_ansible_cloud_product($cmd, $cloud_hook_config) {
	global $event;
	global $htvcenter_SERVER_BASE_DIR;
	global $htvcenter_SERVER_IP_ADDRESS;
	global $htvcenter_EXEC_PORT;
	...
	$event->log("htvcenter_ansible_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-ansible-cloud-product-hook.php", "Handling ".$cmd." event", "", "", 0, 0, 0);
	switch($cmd) {
			case "add":
				...
				foreach ($ansible_group_array as $index => $ansible_app) {
					$event->log("htvcenter_ansible_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-ansible-cloud-product-hook.php", "Adding application ".$ansible_app." as cloud-product", "", "", 0, 0, 0);
			...
			case "remove":
				...
				if (file_exists($cloud_product_class)) {
					...
					foreach ($cloud_selector_id_ar as $key => $value) {
						...
						if ($pos !== false) {
							$event->log("htvcenter_ansible_cloud_product", $_SERVER['REQUEST_TIME'], 5, "htvcenter-ansible-cloud-product-hook.php", "Removing application ".$cloud_selector->quantity." from cloud-products", "", "", 0, 0, 0);
