...
var $lang = array();
	...
	function __construct($htvcenter, $response) {
		...
		require_once($this->htvcenter->get('basedir').'/plugins/ansible/web/class/ansible.class.php');
	...
	function action($action = null) {
		...
		$t = $this->response->html->template($this->tpldir.'/ansible-select.tpl.php');
	...
	function select() {
		...
		$table = $this->response->html->tablebuilder('ansible', $this->response->get_array($this->actions_name, 'select'));
		$table->offset = 0;
		...
		$h['playbooks']['title']    = $this->lang['playbooks'];
		$h['playbooks']['sortable'] = false;
		$h['edit']['title']    = '&#160;';
		...
		foreach($result as $k => $v) {
			...
			$playbooks = $this->ansible->get_playbooks($v['appliance_name']);
			$tmp['playbooks'] = implode(', ', $playbooks);
