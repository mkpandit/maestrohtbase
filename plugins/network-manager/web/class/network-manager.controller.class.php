...
var $lang = array(
	...
	//--------------------------------------------
	function api() {
		require_once($this->htvcenter->get('basedir').'/plugins/network-manager/web/class/network-manager.api.class.php');
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->htvcenter->get('basedir').'/plugins/network-manager/web/class/network-manager.select.class.php');
			$controller = new network_manager_select($this->htvcenter, $this->response, $this);
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->htvcenter->get('basedir').'/plugins/network-manager/web/class/network-manager.add.class.php');
			$controller                  = new network_manager_add($this->htvcenter, $this->response, $this);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->htvcenter->get('basedir').'/plugins/network-manager/web/class/network-manager.remove.class.php');
			$controller                  = new network_manager_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function __reload( $statfile, $resource ) {
		if(isset($resource) && isset($resource->state) && $resource->state === 'active') {
			sleep(2);
			$command = $this->htvcenter->get('basedir').'/plugins/network-manager/bin/htvcenter-network-manager post_config';
