...
var $lang = array();
	...
	function __construct($htvcenter, $response) {
		...
		require_once($this->htvcenter->get('basedir').'/plugins/tmpfs-storage/web/class/tmpfs-storage-volume.class.php');
	}
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/tmpfs-storage-remove.tpl.php');
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($volume_names as $key => $volume_name) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$tmpfs_storage_volume->remove_by_name($volume_name);
