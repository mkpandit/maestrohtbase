		...
		switch($cmd) {
			case "start":
					...
					if (strlen($authblocker->id)) {
						$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Removing authblocker for image $image_name", "", "", 0, 0, 0);
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $ip_storage_id:$ip_storage_ip:$ip_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if (strlen($transfer_from_nfs_param)) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Transfer-to-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if ($run_disable_deployment_export == 1) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the deployment exports when resource $appliance->resources is fully up.", "", "", 0, 0, $appliance_id);
				}
			...
			case "stop":
				...
				$image_authentication->add($image_auth_ar);
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the root-fs exports when resource $appliance->resources is idle again.", "", "", 0, 0, $appliance_id);
				break;
	...
	*/
	...
	function storage_auth_deployment_stop($image_id) {
		...
		if (strlen($transfer_from_nfs_param)) {
			...
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-tmpfs-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
