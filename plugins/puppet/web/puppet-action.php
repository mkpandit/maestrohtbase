...
$cloud_product_hook = $RootDir.'/plugins/puppet/htvcenter-puppet-cloud-product-hook.php';
...
if ($htvcenter_USER->role != "administrator") {
	$event->log("authorization", $_SERVER['REQUEST_TIME'], 1, "puppet-action", "Un-Authorized access to puppet-actions from $htvcenter_USER->name", "", "", 0, 0, 0);
	exit();
}
	...
	switch ($puppet_command) {
		...
		case 'init':
			...
			if (file_exists($cloud_product_hook)) {
				...
				htvcenter_puppet_cloud_product("add", NULL);
			}
			break;
		...
		case 'uninstall':
			...
			if (file_exists($cloud_product_hook)) {
				require_once $cloud_product_hook;
				htvcenter_puppet_cloud_product("remove", NULL);
			}
			break;
