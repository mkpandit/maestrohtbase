...
var $lang = array();
	...
	function action() {
		...
		$t = $response->html->template($this->tpldir.'/personalize-node.select.tpl.php');
		
		$t->add($this->response->html->thisfile, "thisfile");
		$t->add($content);
		$t->add($response->add_node, 'add_node');
	...
	function select() {
		...
		$result = $this->scan_dir($this->node_path);
		$b = array();
		
		$x          = $response->html->a();
		$x->href    = $response->get_url($this->actions_name, 'add' );
		$x->label   = "Add Node"; //$this->lang['action_edit'];
		...
		if(empty($result)) {
			$tab .= "<tr><td>Currently no node found.</td></tr>";
		} else {
			$count = 1;
			foreach($result as $v) {
				$a          = $response->html->a();
