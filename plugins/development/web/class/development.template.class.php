...
var $lang;
	...
	function action($action = null) {
		...
		$t = $this->response->html->template($this->tpldir.'/development-template.tpl.php');
		$t->add($vars);
	...
	function edit($template) {
		...
		require_once($this->htvcenter->get('webdir').'/class/htmlobjects/htmlobject.response.class.php');
		$response = new htmlobject_response($this->response->html, $this->response->id);
		$response->html->debug();

		require_once($this->htvcenter->get('basedir').'/plugins/development/web/class/dummyuser.class.php');
		$dummy = new dummyuser('xxx');

		require_once($this->htvcenter->get('basedir').'/web/base/class/htvcenter.class.php');
		$oq = new htvcenter($this->file, $dummy, $response);
