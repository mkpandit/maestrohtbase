	...
	function select() {
		...
		if(is_array($result)) {
			foreach($result as $v) {
				...
				$tmp['template'] = '&#160;';
				$tmp['hooks'] = '&#160;';
				$path = $this->htvcenter->get('basedir').'/plugins/'.$v.'/web/class/';
		...
		if(is_array($result)) {
			foreach($result as $v) {
				...
				$tmp['hooks'] = '&#160;';
				if($v !== 'plugins') {
					$path = $this->htvcenter->get('basedir').'/web/base/server/'.$v.'/class/';
				}
				elseif ($v === 'plugins') {
					$path = $this->htvcenter->get('basedir').'/web/base/plugins/aa_plugins/class/';
				}
				$files = $this->file->get_files($path);
				$oop = false;
				foreach($files as $file) {
