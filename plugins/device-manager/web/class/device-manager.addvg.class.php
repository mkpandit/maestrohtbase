...
var $lang = array(
	...
	function addvg() {
		// load partition file
		$file = $this->statfile;
		$command = $this->htvcenter->get('basedir').'/plugins/device-manager/bin/htvcenter-device-manager list';
		...
		if(!$form->get_errors() && $response->submit()) {
			if($response->html->request()->get('confirm') !== '') {
				$file = $this->statfile;
				$command  = $this->htvcenter->get('basedir').'/plugins/device-manager/bin/htvcenter-device-manager add_vg';
