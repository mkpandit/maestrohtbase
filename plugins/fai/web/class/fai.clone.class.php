...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/fai-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$fai_volume->get_instance_by_name($name);
			if (isset($fai_volume->id) && $fai_volume->id > 0) {
			...
			if(isset($error)) {
				...
				// add volume
				$fai_volume->get_instance_by_name($this->volume);
				$volume_fields = array();
				$volume_fields["fai_volume_id"] = (int)str_replace(".", "", str_pad(microtime(true), 15, "0"));
				$volume_fields['fai_volume_name'] = $name;
				$volume_fields['fai_volume_root'] = $fai_volume->root;
				$volume_fields['fai_volume_description'] = $fai_volume->description;
				$fai_volume->add($volume_fields);
				// add image
	...
	function get_response() {
		...
		$d['name']['object']['attrib']['customattribs'] = 'data-prefix="fai_" data-length="8"';
