...
function create_clone_lvm_aoe_deployment($cloud_image_id, $image_clone_name, $disk_size) {
	...
	$image_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/lvm-storage/bin/htvcenter-lvm-storage snap -n ".$image_location_name." -v ".$volume_group." -t lvm-aoe-deployment -s ".$image_clone_name." -m ".$disk_size." --htvcenter-cmd-mode background";
...
function remove_lvm_aoe_deployment($cloud_image_id) {
	...
	$image_location_name=substr($image_rootdevice_rest, 0, $ident_separate2);
	$root_device=substr($image_rootdevice_rest, $ident_separate2+1);
	$image_remove_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/lvm-storage/bin/htvcenter-lvm-storage remove -n ".$image_location_name." -v ".$volume_group." -t lvm-aoe-deployment --htvcenter-cmd-mode background";
...
function resize_lvm_aoe_deployment($cloud_image_id, $resize_value) {
	...
	$image_location_name=substr($image_rootdevice_rest, 0, $ident_separate2);
	$root_device=substr($image_rootdevice_rest, $ident_separate2+1);
	$image_resize_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/lvm-storage/bin/htvcenter-lvm-storage resize -n ".$image_location_name." -v ".$volume_group." -m ".$resize_value." -t lvm-aoe-deployment --htvcenter-cmd-mode background";
...
function create_private_lvm_aoe_deployment($cloud_image_id, $private_disk, $private_image_name) {
	...
	$image_location_name=substr($image_rootdevice_rest, 0, $ident_separate2);
	$root_device=substr($image_rootdevice_rest, $ident_separate2+1);
	$image_resize_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/lvm-storage/bin/htvcenter-lvm-storage clone -n ".$image_location_name." -s ".$private_image_name." -v ".$volume_group." -m ".$private_disk." -t lvm-aoe-deployment -u ".$htvcenter_admin_user->name." -p ".$htvcenter_admin_user->password." --htvcenter-cmd-mode background";
	$event->log("cloud", $_SERVER['REQUEST_TIME'], 5, "lvm-aoe-deployment-cloud-hook", "Running : $image_resize_cmd", "", "", 0, 0, 0);
	$resource->send_command($resource_ip, $image_resize_cmd);
