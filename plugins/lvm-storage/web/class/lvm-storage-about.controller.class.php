...
var $lang = array(
	'documentation' => array (
		...
		'requirements_list' => '<ul><li>A resource for the LVM-Storage Storage (this can be a remote system integrated into htvcenter e.g. via the "local-server" plugin or the htvcenter server itself)</li>
	...
	'bootservice' => array (
		...
		'boot_service_content' => 'The LVM-Storage Plugin provides an htvcenter Boot-Service.
			This "LVM-Storage Boot-Service" is automatically downloaded and executed by the htvcenter-Client on all integrated Systems.
			The Boot-Service is located at:<br>
			<br>
				<i><b>/usr/share/htvcenter/plugins/lvm-storage/web/boot-service-lvm-storage.tgz</b></i>
			<br>
			<br>
			...
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service view -n lvm-storage -a default</b></i>
			<br>
			<br>
			To view a Boot-Service configuration of a specific appliance run:<br>
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service view -n lvm-storage -a [appliance-name]</b></i>
			<br>
			<br>
			To adapt a parameter in the current default Boot-Service configuration run:<br>
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service configure -n lvm-storage -a default -k [key] -v [value]</b></i>
			<br>
			<br>
			To adapt a paramter in the Boot-Service configuration of a specific appliance run:<br>
			<br>
				<i><b>/usr/share/htvcenter/bin/htvcenter boot-service configure -n lvm-storage -a [appliance-name] -k [key] -v [value]</b></i>
			<br>
			<br>
			In case the openmQRM Server itself is used as the LVM-Storage Storage please edit:<br>
			<br>
				<i><b>/usr/share/htvcenter/plugins/lvm-storage/etc/htvcenter-plugin-lvm-storage.conf</b></i>
			<br>
			<br>
	...
	function documentation( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new lvm_storage_about_documentation($this->htvcenter, $this->response);
	...
	function bootservice( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new lvm_storage_about_bootservice($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
