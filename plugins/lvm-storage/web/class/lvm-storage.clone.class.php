...
var $lang = array();
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->htvcenter->get('basedir').'/plugins/lvm-storage/bin/htvcenter-lvm-storage clone';
			...
			$statfile = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$this->resource->id.'.'.$this->volgroup.'.lv.stat';
			...
			if(isset($error)) {
				...
				$file = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$resource->id.'.lvm.'.$name.'.sync_progress';
				if(file_exists($file)) {
					unlink($file);
				}
				...
				switch($this->deployment->type) {
					case 'lvm-aoe-deployment':
						...
						$ident_file = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$this->resource->id.'.lv.lvm-aoe-deployment.ident';
						$get_ident_command  = $this->htvcenter->get('basedir').'/plugins/lvm-storage/bin/htvcenter-lvm-storage post_identifier';
