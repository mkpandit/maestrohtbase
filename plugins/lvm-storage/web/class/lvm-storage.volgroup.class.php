...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
	}
	...
	function edit() {
		...
		//	$d['icon'] = "<img width=24 height=24 src=$resource_icon_default>";
			...
			// Volgroup info
			$lines = explode("\n", file_get_contents($this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$this->resource->id.'.vg.stat'));
					...
					foreach($lines as $line) {
						if($line !== '') {
							...
							if ($line[4] == "clone_in_progress") {
								...
								// progressbar
								$t->add(uniqid('b'), 'id');
								$t->add($this->htvcenter->get('baseurl').'/api.php?action=plugin&plugin=lvm-storage&lvm_storage_action=progress&name='.$this->resource->id.'.lvm.'.$name.'.sync_progress', 'url');
