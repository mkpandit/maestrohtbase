			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->htvcenter->get('basedir').'/plugins/lvm-storage/bin/htvcenter-lvm-storage remove';
						$command .= ' -n '.$lvol;
						...
						$file = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
						if(file_exists($file)) {
							unlink($file);
						}
