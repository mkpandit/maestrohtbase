...
var $lang = array();
	...
	function snap() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command     = $this->htvcenter->get('basedir').'/plugins/lvm-storage/bin/htvcenter-lvm-storage snap';
	...
	function set_max() {
		...
		$statfile = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$this->resource->id.'.vg.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
