			...
			switch ($image_command) {
				case 'add':
					...
					if (strlen($image->id)) {
						...
						$ident_file = $this->htvcenter->get('basedir').'/plugins/lvm-storage/web/storage/'.$this->resource->id.'.lvm.'.$image_name.'.adapt';
						$get_ident_command  = $this->htvcenter->get('basedir').'/plugins/lvm-storage/bin/htvcenter-lvm-storage adapt';
