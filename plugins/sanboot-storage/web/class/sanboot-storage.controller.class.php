...
var $lang = array(
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new sanboot_storage_select($this->htvcenter, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_edit($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function addvg( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/sanboot-storage/class/sanboot-storage.addvg.class.php');
			$controller                  = new sanboot_storage_addvg($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
	...
	function removevg( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/sanboot-storage/class/sanboot-storage.removevg.class.php');
			$controller                  = new sanboot_storage_removevg($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->rootdir.'/plugins/device-manager/tpl/';
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function volgroup( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_volgroup($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                = new sanboot_storage_add($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function image( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/sanboot-storage/class/sanboot-storage.image.class.php');
			$controller                  = new sanboot_storage_image($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function snap( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_snap($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_clone($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function resize( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new sanboot_storage_resize($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function __reload($mode) {
		...
		// reload volume group
		if($mode === 'vg') {
			$file = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.vg.stat';
			$command .= $htvcenter_SERVER_BASE_DIR.'/plugins/sanboot-storage/bin/htvcenter-sanboot-storage post_vg';
		}
		// reload logical volumes
		if($mode === 'lv') {
			$file = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.'.$volgroup.'.lv.stat';
			$command .= $htvcenter_SERVER_BASE_DIR.'/plugins/sanboot-storage/bin/htvcenter-sanboot-storage post_lv';
