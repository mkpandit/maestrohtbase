...
var $lang = array();
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/bin/htvcenter-sanboot-storage clone';
			...
			if(isset($error)) {
				...
				$file = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.sanboot.'.$name.'.sync_progress';
				if(file_exists($file)) {
					unlink($file);
				}
