<?php
/**
 * sanboot-Storage Remove Volume(s)
 *
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/sanboot-storage-remove.tpl.php');
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/bin/htvcenter-sanboot-storage remove';
						$command .= ' -n '.$lvol;
						...
						$file = $this->htvcenter->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
						if(file_exists($file)) {
							unlink($file);
						}
