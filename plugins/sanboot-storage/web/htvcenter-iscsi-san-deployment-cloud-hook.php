...
function create_clone_iscsi_san_deployment($cloud_image_id, $image_clone_name, $disk_size) {
	...
	$image_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/sanboot-storage/bin/htvcenter-sanboot-storage snap -n ".$image_location_name." -v ".$volume_group." -t iscsi-san-deployment -s ".$image_clone_name." -m ".$disk_size." -i ".$image_password." --htvcenter-cmd-mode background";
...
function remove_iscsi_san_deployment($cloud_image_id) {
	...
	$ident_separate1=strpos($image_location_name_full, ":");
	$image_location_name=substr($image_location_name_full, 0, $ident_separate1);
	$image_remove_clone_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/sanboot-storage/bin/htvcenter-sanboot-storage remove -n ".$image_location_name." -v ".$volume_group." -t iscsi-san-deployment --htvcenter-cmd-mode background";
...
function resize_iscsi_san_deployment($cloud_image_id, $resize_value) {
	...
	$ident_separate1=strpos($image_location_name_full, ":");
	$image_location_name=substr($image_location_name_full, 0, $ident_separate1);
	$image_resize_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/sanboot-storage/bin/htvcenter-sanboot-storage resize -n ".$image_location_name." -v ".$volume_group." -m ".$resize_value." -t iscsi-san-deployment --htvcenter-cmd-mode background";
...
function create_private_iscsi_san_deployment($cloud_image_id, $private_disk, $private_image_name) {
	...
	$ident_separate1=strpos($image_location_name_full, ":");
	$image_location_name=substr($image_location_name_full, 0, $ident_separate1);
	$image_resize_cmd=$htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/sanboot-storage/bin/htvcenter-sanboot-storage clone -n ".$image_location_name." -s ".$private_image_name." -v ".$volume_group." -m ".$private_disk." -t iscsi-san-deployment -u ".$htvcenter_admin_user->name." -p ".$htvcenter_admin_user->password." --htvcenter-cmd-mode background";
	$event->log("cloud", $_SERVER['REQUEST_TIME'], 5, "iscsi-san-deployment-cloud-hook", "Running : $image_resize_cmd", "", "", 0, 0, 0);
	$resource->send_command($resource_ip, $image_resize_cmd);
