...
var $lang = array();
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command     = $this->htvcenter->get('basedir')."/plugins/nfs-storage/bin/htvcenter-nfs-storage add -n $name";
	...
	function get_response() {
		...
		$d['name']['object']['attrib']['customattribs'] = 'data-prefix="nfs_" data-length="8"';
	...
	//--------------------------------------------
	function set_max() {
		$statfile = $this->htvcenter->get('basedir').'/plugins/nfs-storage/web/storage/'.$this->resource->id.'.nfs.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
