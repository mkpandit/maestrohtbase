...
var $lang = array(
	...
	'edit' => array (
		...
		'manual_configured' => 'Storage is manually configured and will not be administrated by htvcenter',
	),
	'add' => array (
		'tab' => 'Add NFS Volume',
	...
	'manual' => array (
		...
		'explanation_1' => 'In case the NFS Storage server is not managed by htvcenter please use this form to manually create the list of exported paths to server-images on the NFS Storage server.',
		'explanation_2' => 'Please notice that in case a manual configuration exist htvcenter will not send any automated Storage-authentication commands to this NFS Storage-server!',
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new nfs_storage_select($this->htvcenter, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new nfs_storage_edit($this->htvcenter, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                = new nfs_storage_add($this->htvcenter, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function image( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new nfs_storage_image($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new nfs_storage_remove($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new nfs_storage_clone($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function auth( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new nfs_storage_auth($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function manual( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new nfs_storage_manual($this->htvcenter, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload() {
		...
		$command  = $htvcenter_SERVER_BASE_DIR."/plugins/nfs-storage/bin/htvcenter-nfs-storage post_exports";
		...
		$file = $this->htvcenter->get('basedir').'/plugins/nfs-storage/web/storage/'.$resource->id.'.nfs.stat';
