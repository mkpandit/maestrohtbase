...
global $event;
  ...
  /*      if (file_exists("$RootDir/plugins/cloud/.running")) {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Found Cloud enabled and running. Checking for CloudNAT", "", "", 0, 0, $appliance_id);
			...
			if (!strcmp($cn_nat_enabled, "true")) {
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Found CloudNAT enabled, translated $internal_resource_ip to $resource_ip", "", "", 0, 0, $appliance_id);
			} else {
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Cloudnat is disabled, keeping $resource_ip", "", "", 0, 0, $appliance_id);
			}

		} else {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Cloud is not enabled/running. Not checking for CloudNAT", "", "", 0, 0, $appliance_id);

		}
*/

		// manual configured ?
		$StorageDir = $_SERVER["DOCUMENT_ROOT"].'/htvcenter/base/plugins/nfs-storage/storage';
		...
		if (file_exists($statfile_manual)) {
			...
			if (strlen($authblocker->id)) {
				$event->log('storage_auth_function', $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Removing authblocker for image $image_name", "", "", 0, 0, 0);
				$authblocker->remove($authblocker->id);
			}
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "NFS Storage $storage->id is manually configured. Skipping automatic authentication hook ...", "", "", 0, 0, $appliance_id);
			return;
		}
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Authenticating $image_name to resource $resource_ip", "", "", 0, 0, $appliance_id);
				$auth_start_cmd = $htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/".$deployment_plugin_name."/bin/htvcenter-".$deployment_plugin_name." auth -n ".$image_name." -r ".$image_rootdevice." -i ".$resource_ip." -u ".$htvcenter_admin_user->name." -p ".$htvcenter_admin_user->password." --htvcenter-cmd-mode background";
				...
				if (strlen($install_from_nfs_param)) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $ip_storage_id:$ip_storage_ip:$ip_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if (strlen($transfer_from_nfs_param)) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Transfer-to-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
				...
				if ($run_disable_deployment_export == 1) {
					...
					$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the deployment exports when resource $appliance->resources is fully up.", "", "", 0, 0, $appliance_id);
				}
			...
			case "stop":
				...
				$image_authentication->add($image_auth_ar);
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Registered image $appliance->imageid for de-authentication the root-fs exports when resource $appliance->resources is idle again.", "", "", 0, 0, $appliance_id);
				break;
	...
	*/
	...
	function storage_auth_stop($image_id) {
		...
		$auth_stop_cmd = $htvcenter_SERVER_BASE_DIR."/htvcenter/plugins/".$deployment_plugin_name."/bin/htvcenter-".$deployment_plugin_name." auth -r ".$image_rootdevice." -i ".$htvcenter_SERVER_IP_ADDRESS." --htvcenter-cmd-mode background";
		$resource = new resource();
	...
	*/
	...
	function storage_auth_deployment_stop($image_id) {
		...
		if (strlen($transfer_from_nfs_param)) {
			...
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "htvcenter-nfs-deployment-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
